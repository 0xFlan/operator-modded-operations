...
[BepInPlugin("operator.modded-operations", "OPERATOR: Modded Operations", "0.3.17")]
[BepInProcess("OPERATOR.exe")]
[BepInDependency("operator.modapi", "0.2.0-alpha.2")]
public sealed class CerberusNativeTabFix : BasePlugin
{
	...
	public override void Load()
	{
		...
		if (!string.Equals(OperatorApi.ApiVersion, "0.2.0-alpha.2", StringComparison.Ordinal))
		{
			((BasePlugin)this).Log.LogError((object)("Modded Operations requires Operator Mod API 0.2.0-alpha.2, but Core exposes " + OperatorApi.ApiVersion + ". Adapter startup was refused."));
			return;
		}
	...
	private void ApplyStandaloneRenderContract(Scene scene, ActiveMapOperation operation)
	{
		...
		try
		{
			...
			((Component)val).transform.rotation = (flag ? new Quaternion(0.1168685f, 0.6386099f, -0.6774567f, 0.3457913f) : new Quaternion(0.78155714f, 0.20589164f, -0.10890977f, 0.57871914f));
			val.color = Color.white;
			val.useColorTemperature = true;
			val.colorTemperature = (flag ? 9754f : 6727f);
			val.intensity = (flag ? 40f : 52241.375f);
			val.bounceIntensity = (flag ? 1f : 1.5f);
			...
			((VolumeParameter<float>)(object)obj7.fixedExposure).Override(8.32f);
			((VolumeParameter<float>)(object)obj7.compensation).Override(flag ? 1.16f : 0f);
			((VolumeParameter<float>)(object)obj7.limitMin).Override(flag ? 5.065282f : 11.660274f);
			((VolumeParameter<float>)(object)obj7.limitMax).Override(flag ? 9.348571f : 12.358905f);
			((VolumeParameter<float>)(object)obj7.adaptationSpeedDarkToLight).Override(3f);
			((VolumeParameter<float>)(object)obj7.adaptationSpeedLightToDark).Override(3f);
			...
			Bloom obj8 = val5.Add<Bloom>(false);
			((VolumeParameter<int>)(object)((VolumeComponentWithQuality)obj8).quality).Override(flag ? 1 : 3);
			((VolumeParameter<float>)(object)obj8.intensity).Override(flag ? 0.3f : 0.359f);
			((VolumeParameter<float>)(object)obj8.threshold).Override(0.9f);
			((VolumeParameter<float>)(object)obj8.scatter).Override(flag ? 0.2f : 0.866f);
			((VolumeParameter<bool>)(object)obj8.anamorphic).Override(!flag);
			((VolumeParameter<BloomResolution>)(object)obj8.m_Resolution).Override((BloomResolution)2);
			ColorAdjustments obj9 = val5.Add<ColorAdjustments>(false);
			((VolumeParameter<float>)(object)obj9.postExposure).Override(flag ? 0f : (-0.3f));
			((VolumeParameter<float>)(object)obj9.contrast).Override(flag ? 17.3f : 30f);
			((VolumeParameter<float>)(object)obj9.saturation).Override(flag ? 22f : (-22f));
			if (!flag)
			{
				WhiteBalance obj10 = val5.Add<WhiteBalance>(false);
				((VolumeParameter<float>)(object)obj10.temperature).Override(10f);
				((VolumeParameter<float>)(object)obj10.tint).Override(10f);
			}
			IndirectLightingController obj11 = val5.Add<IndirectLightingController>(false);
			((VolumeParameter<float>)(object)obj11.indirectDiffuseLightingMultiplier).Override(flag ? 1f : 2f);
			...
			HDShadowSettings obj12 = val5.Add<HDShadowSettings>(false);
			((VolumeParameter<float>)(object)obj12.maxShadowDistance).Override(200f);
			((VolumeParameter<float>)(object)obj12.cascadeShadowSplit0).Override(0.05f);
			log.LogInfo((object)("Standalone render contract applied from package-owned scene plus its explicit native-outdoor-v1 profile marker: time=" + operation.TimeCode + ", sunLux=" + val.intensity + ", nightAmbient=" + flag + ", whitePhosphor=" + operation.WhitePhosphorApplied + ", externalLut=" + ((Object)(object)val7 != (Object)null) + "."));
		}
		catch (Exception ex)
		{
	...
	private void TrySpawnStandalonePveEnemies(ActiveMapOperation operation)
	{
		...
		try
		{
			...
			{
				ConfigureStandaloneBotDetails(((Component)item).GetComponent<BotSpawnDetails>() ?? ((Component)item).gameObject.AddComponent<BotSpawnDetails>());
			...
			log.LogInfo((object)("Standalone PVE released a server-owned AI population through shipped RaidManager.ServerSpawnAI: count=" + num + ", requestedRange=" + minimumEnemies + "-" + maximumEnemies + ", chosen=" + val10 + ", markers=" + list.Count + ", firearmCapablePrefabs=" + list2.Count + "."));
		}
		catch (Exception ex)
		{
