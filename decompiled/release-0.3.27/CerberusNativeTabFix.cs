...
[BepInPlugin("operator.modded-operations", "OPERATOR: Modded Operations", "0.3.27")]
...
public sealed class CerberusNativeTabFix : BasePlugin
{
	...
	private sealed class ActiveMapOperation
	{
		...
		public bool PvePreexistingBrainsCaptured;
	...
	private void TrySpawnStandalonePveEnemies(ActiveMapOperation operation)
	{
		...
		try
		{
			...
			int num = Math.Min(val2, list.Count);
			GameMode gameModeComponent = operation.GameModeComponent;
			...
			pveRaidManager.exfilZones = new List<ExfilZone>();
			pveRaidManager.exfilZones.Add(operation.PveExfilZone);
			if (!TryValidateStandalonePveSpawnedTeamContract(operation, val, num, out var summary))
			{
				operation.PveTeamContractFailed = true;
				throw new InvalidOperationException("native spawned-team contract failed: " + summary);
			}
			operation.PveTeamContractValidated = true;
			operation.PveEnemyCount = num;
			StartProfiledPveAiDiagnostics(operation, val);
			log.LogInfo((object)("Standalone PVE released a server-owned AI population through shipped RaidManager.ServerSpawnAI: count=" + num + ", requestedRange=" + minimumEnemies + "-" + maximumEnemies + ", chosen=" + val2 + ", markers=" + list.Count + ", firearmCapablePrefabs=" + firearmCapablePrefabs + ", selectedCohortPrefabs=" + prefabs.Count + ", playerTeam=" + teamId + ", hostileTeam=" + selected.TeamId + ", hostileTeamName=" + selected.TeamName + ", hostileTeamStatsInstance=" + selected.StartingTeamStatsInstanceId + ", teamContract=" + summary + ", raidExfilsAfterNativeSpawn=" + pveRaidManager.exfilZones.Count + ", aiProfile=" + FormatPveAiProfile(pveAiProfile) + "."));
		}
		catch (Exception ex)
		{
			DestroyNewStandalonePveBrains(operation, val);
	...
	private bool TrySelectStandalonePvePrefabCohort(GameManager gameManager, int playerTeamId, out StandalonePvePrefabCohort selected, out int firearmCapablePrefabs, out string cohortSummary, out string failure)
	{
		...
		for (int i = 0; i < val.Count; i++)
		{
			...
			try
			{
				...
				TargetList val7 = ((component != null) ? component.TargetPool : null);
				int num2 = (((Object)(object)val6 == (Object)null) ? (-1) : val6.ThisTeamId);
				if ((Object)(object)component == (Object)null || (Object)(object)component2 == (Object)null || (Object)(object)val6 == (Object)null || num2 == -1 || val7 == null || (Object)(object)component2.MyTeamIdentifier != (Object)(object)component || component2.TargetPool != val7 || (Object)(object)val3.MyTeamIdentifierReference != (Object)(object)component2)
				{
					num++;
				}
				else
	...
	{
		List<BrainAI> newStandalonePveBrains = GetNewStandalonePveBrains(operation, gameManager);
		if (newStandalonePveBrains.Count != expectedCount)
		{
			summary = "spawnedBrainCount=" + newStandalonePveBrains.Count + ", expected=" + expectedCount;
			return false;
		}
		int num = 0;
		int num2 = 0;
		...
		int num6 = 0;
		int num7 = 0;
		HashSet<int> hashSet = new HashSet<int>(from brain in newStandalonePveBrains
		...
		foreach (BrainAI item in newStandalonePveBrains)
		{
			if ((Object)(object)item == (Object)null)
			{
				num2++;
			...
			try
			{
				...
				{
					if (IsTargetOwnedByStandalonePveCohort(item.possibleTargets[num10], newStandalonePveBrains))
					{
						num6++;
					}
				}
		...
		summary = "team=" + operation.PveExpectedTeamId + ", teamName=" + operation.PveExpectedTeamName + ", teamStatsInstance=" + operation.PveExpectedStartingTeamStatsInstanceId + ", playerTeam=" + operation.PvePlayerTeamId + ", brains=" + newStandalonePveBrains.Count + "/" + expectedCount + ", managerTeamMatches=" + flag + ", repairedUnresolved=" + num + ", identifierFailures=" + num2 + ", teamStateFailures=" + num3 + ", teamStatsFailures=" + num4 + ", friendlyRelationships=" + num7 + ", cohortInEnemyLists=" + num5 + ", cohortInPossibleTargets=" + num6;
		return result;
	}

	private static List<BrainAI> GetNewStandalonePveBrains(ActiveMapOperation operation, GameManager gameManager)
	{
		List<BrainAI> list = new List<BrainAI>();
		if (operation == null || !operation.PvePreexistingBrainsCaptured || ((gameManager != null) ? gameManager.allAI : null) == null)
		{
			return list;
		}
		for (int i = 0; i < gameManager.allAI.Count; i++)
		{
			...
			if ((Object)(object)val != (Object)null && !operation.ProfiledPvePreexistingBrainIds.Contains(((Object)val).GetInstanceID()))
			{
				list.Add(val);
			}
		}
	...
	private void DestroyNewStandalonePveBrains(ActiveMapOperation operation, GameManager gameManager)
	{
		List<BrainAI> newStandalonePveBrains = GetNewStandalonePveBrains(operation, gameManager);
		foreach (BrainAI item in newStandalonePveBrains)
		{
		...
		if (newStandalonePveBrains.Count > 0)
		{
			log.LogWarning((object)("Standalone PVE removed " + newStandalonePveBrains.Count + " newly spawned bots after its native team contract failed."));
		}
	...
	private static void CaptureProfiledPvePreexistingBrains(ActiveMapOperation operation, GameManager gameManager)
	{
		...
		operation.PvePreexistingBrainsCaptured = true;
	}
	...
	private static void ResetProfiledPveGenerationState(ActiveMapOperation operation)
	{
		...
		{
			operation.ProfiledPvePreexistingBrainIds.Clear();
			operation.PvePreexistingBrainsCaptured = false;
