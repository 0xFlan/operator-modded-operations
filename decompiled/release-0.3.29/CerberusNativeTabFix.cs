...
[BepInDependency("operator.modapi", "0.2.0-alpha.6")]
public sealed class CerberusNativeTabFix : BasePlugin
{
	...
	private sealed class ActiveMapOperation
	{
		...
		public readonly Dictionary<uint, NetworkIdentity> PveOwnedServerIdentities = new Dictionary<uint, NetworkIdentity>();

		public readonly HashSet<int> PveOwnedBrainInstanceIds = new HashSet<int>();
		...
		public readonly HashSet<int> ProfiledPveReactionTimeHandledBrainIds = new HashSet<int>();
	...
	private sealed class StandalonePvpGameMode : PvpGameode
	{
		...
		{
			if (instance == null || !instance.TryClaimStandalonePvpAllPlayersLoaded(this))
			{
				return;
			}
			try
			...
			{
				instance?.OnStandalonePvpAllPlayersLoadedFailed(this, exception);
			}
		}
		...
		private void EnterNativeReadiness(string source)
		{
			...
			{
				instance.MarkStandalonePvpReadinessInitializationFailed(this, source, exception);
			}
		}
	}
	...
	private sealed class HostPvpAgreement
	{
		...
		public readonly Dictionary<int, NetworkConnectionToClient> RequiredConnections = new Dictionary<int, NetworkConnectionToClient>();

		public readonly HashSet<int> ContentReadyConnectionIds = new HashSet<int>();

		public readonly Dictionary<int, ulong> SceneReadyEpochByConnectionId = new Dictionary<int, ulong>();
	...
	private const string PvpAgreementCapabilities = "exact-content-v2;runtime-binary-sha256-v1;package-runtime-ready-v1;remote-preload-v1;scene-ready-epoch-v1;native-pvp-v1";

	private const ushort PvpAgreementProtocolVersion = 2;

	private const ushort PvpAgreementMessageId = 19791;
	...
	private const double PvpContentReadyTimeoutSeconds = 45.0;

	private const double PvpSceneReadyTimeoutSeconds = 90.0;

	private const double PvpSceneReadyRequestRetrySeconds = 5.0;

	private const double PvpNativeOwnerTimeoutSeconds = 90.0;

	private const double PvpNativeLifecycleTimeoutSeconds = 90.0;

	private const string PvpNoCompanionIdentity = "none";

	private const string PvpApiPluginGuid = "operator.modapi";

	private const string PvpApiHostTypeName = "OperatorModAPI.BepInEx.OperatorApiPlugin";
	...
	public override void Load()
	{
		...
		if (!string.Equals(OperatorApi.ApiVersion, "0.2.0-alpha.6", StringComparison.Ordinal))
		{
			((BasePlugin)this).Log.LogError((object)("Modded Operations requires Operator Mod API 0.2.0-alpha.6, but Core exposes " + OperatorApi.ApiVersion + ". Adapter startup was refused."));
			return;
		}
	...
	public override bool Unload()
	{
		...
		try
		{
			...
			if (activeMapOperation != null)
			{
				ModdedOperationDefinition operation = activeMapOperation.Operation;
				if ((int)((operation != null) ? new ModdedOperationMode?(operation.Mode) : ((ModdedOperationMode?)null)).GetValueOrDefault() == 1)
				{
					if (activeOperation.SceneHandle == 0 && !activeOperation.NativeLaunchInvoked)
					{
						RemotePvpAgreement obj = remotePvpAgreement;
						if (obj == null || !obj.ContentCommitted)
						{
							HostPvpAgreement obj2 = hostPvpAgreement;
							if (obj2 == null || !obj2.NativeLaunchInvoked)
							{
								goto IL_007a;
							}
						}
					}
					obj3 = activeOperation;
					goto IL_0083;
				}
			}
			goto IL_007a;
			IL_0083:
			ActiveMapOperation activeMapOperation2 = (ActiveMapOperation)obj3;
			ReleasePvpPeerAgreementTransport("plugin unload");
			if (activeMapOperation2 != null && !activeMapOperation2.PvpAbortReturnRequested)
			{
				RequestNativePvpAbortReturn(activeMapOperation2);
			}
		...
		catch (Exception ex)
		{
			...
			if (obj6 != null)
			{
				obj6.LogError((object)("Modded Operations unload failed: " + ex));
			}
			return false;
		}
	}
		...
		try
		{
			...
			if (pendingMapLaunch.CurrentRequest == null)
			{
				if (pendingMapLaunch.RequestIndex >= pendingMapLaunch.BundlePaths.Count)
				{
					...
					if (remotePvpAgreement != null && !NetworkServer.active)
					{
						ModdedMapDefinition map = remotePvpAgreement.Map;
						if (string.Equals((map != null) ? map.Id : null, pendingMapLaunch.Map.Id, StringComparison.Ordinal))
						{
							num = (string.Equals(remotePvpAgreement.Identity?.PackageContentId, pendingMapLaunch.Map.PackageContentId, StringComparison.Ordinal) ? 1 : 0);
							goto IL_0181;
						}
					}
					num = 0;
			...
			try
			{
				num2 = new FileInfo(pendingMapLaunch.BundlePaths[pendingMapLaunch.RequestIndex]).Length;
			}
			catch
			{
			}
			log.LogInfo((object)("Modded Operations loaded verified bundle " + (pendingMapLaunch.RequestIndex + 1) + "/" + pendingMapLaunch.BundlePaths.Count + ": file=" + Path.GetFileName(pendingMapLaunch.BundlePaths[pendingMapLaunch.RequestIndex]) + ", bytes=" + num2 + ", seconds=" + FormatElapsedSeconds(pendingMapLaunch.CurrentRequestStartedTimestamp) + "."));
			...
			NotifyPvpVerifiedMapBundlesAvailable(pendingMapLaunch.Map);
			if (pendingMapLaunch.LaunchRequested && !flag)
			{
	...
	private void FailPendingLaunch(string reason)
	{
		PendingMapLaunch pendingMapLaunch = pendingLaunch;
		pendingLaunch = null;
		NotifyPvpPendingMapLoadFailed(pendingMapLaunch, reason);
	...
	private void InvokeNativeCatalogLaunch(CatalogPresentation presentation, ModdedMapDefinition map, ModdedOperationDefinition operation, string timeCode, MissionLaptop launchLaptop, PlayerNetworking launchPlayer, SceneVariantSelection sceneSelection, bool pvpPeerAgreementSatisfied = false)
	{
		...
		if ((int)operation.Mode != 1)
		{
			ClearPvpPeerAgreementForNonPvpLaunch();
		}
		if (!pvpPeerAgreementSatisfied && (int)operation.Mode == 1)
		{
			BeginPvpPeerAgreementOrFail(presentation, map, operation, timeCode, launchLaptop, launchPlayer, sceneSelection);
			return;
		}
		try
		{
		...
		catch (Exception ex)
		{
			...
			SetNativeConfirmationLoadingState(presentation, loading: false);
			if (activeOperation == activeMapOperation && (activeMapOperation == null || !activeMapOperation.NativeLaunchInvoked))
			{
				activeOperation = null;
			}
	...
	private void InvokeNativeBoardStart(CatalogPresentation presentation, ModdedMapDefinition map, ModdedOperationDefinition operation, string timeCode, MissionLaptop capturedLaptop, PlayerNetworking capturedPlayer, SceneVariantSelection sceneSelection)
	{
		...
		if (activeOperation == null || activeOperation.Operation != operation || activeOperation.Map != map)
		{
			throw new InvalidOperationException("active package operation ownership changed before native start");
		}
	...
	private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
	{
		...
		{
			int sceneHandle = activeMapOperation.SceneHandle;
			if (sceneHandle != 0 && SceneHandle.op_Implicit(sceneHandle) != ((Scene)(ref scene)).handle)
			{
				ResetPvpSceneAgreementForReload(activeMapOperation);
				log.LogInfo((object)("Standalone package restart adopted replacement scene before prior unload: previousHandle=" + sceneHandle + ", replacementHandle=" + ((object)((Scene)(ref scene)).handle/*cast due to constrained. prefix*/).ToString() + "."));
			}
			...
			activeMapOperation.PvpAbortReturnRequested = false;
			...
			activeMapOperation.AllPlayersLoadedClaimed = false;
			...
			ResetProfiledPveGenerationState(activeMapOperation);
			NotifyPvpSceneLoading(activeMapOperation);
			if (!ValidateStandaloneSceneContract(scene, activeMapOperation, out var error))
			{
				...
				NotifyPvpScenePreparationFailed(activeMapOperation, error);
				log.LogError((object)("Standalone package scene contract rejected map=" + activeMapOperation.Map.Id + ", sceneHandle=" + activeMapOperation.SceneHandle + ": " + error + "."));
			}
			else
			{
	...
	private void OnSceneUnloaded(Scene scene)
	{
		...
		{
			activeMapOperation.SceneHandle = 0;
			if (!CompletePvpPeerAgreementOnNativeReturn(activeMapOperation))
			{
				ResetPvpSceneAgreementForReload(activeMapOperation);
			}
			ReleaseStandaloneSceneContracts(activeMapOperation);
	...
	private void PrepareStandaloneScene(Scene scene, ActiveMapOperation operation)
	{
		...
		{
			NotifyPvpScenePreparationFailed(operation, error);
		...
		{
			NotifyPvpScenePreparationFailed(operation, error2);
		...
		if (!ConfigureStandalonePlayerSpawnContract(scene, operation, out var error3) && (int)operation.Operation.Mode == 1)
		{
			operation.TerrainReady = false;
			operation.ScenePreparationComplete = false;
			NotifyPvpScenePreparationFailed(operation, error3);
			log.LogError((object)("Standalone PVP player-spawn contract failed closed: map=" + operation.Map.Id + ", reason=" + error3 + "."));
			ReleaseRuntimeTerrain(operation);
			return;
		}
		...
		{
			NotifyPvpScenePreparationFailed(operation, "standalone game-mode bootstrap was not created");
		}
	...
	private bool ConfigureStandalonePlayerSpawnContract(Scene scene, ActiveMapOperation operation, out string error)
	{
		...
		try
		{
			...
			if (!operation.SpawnContractInstalled || !SameNativeSpawnList(GameManager.SpawnPointsInScene, operation.OwnedSpawnPoints))
			{
				throw new InvalidOperationException("package-owned SpawnPointsInScene identity was not installed");
			}
			if ((int)operation.Operation.Mode == 1)
			{
				GameManager val5 = GameManager.instance;
				if ((Object)(object)val5 == (Object)null)
				{
					throw new InvalidOperationException("GameManager was unavailable for the PVP spawn contract");
				}
				if (!SameNativeGameObjectArray(val5.Pspawns, operation.OwnedFallbackSpawns))
				{
					throw new InvalidOperationException("package-owned Pspawns identity was not installed");
				}
				if (!operation.RandomSpawnsCaptured || operation.OwnedRandomSpawns || val5.RandomSpawns != operation.OwnedRandomSpawns)
				{
					throw new InvalidOperationException("RandomSpawns was not owned as false by this PVP generation");
				}
			}
		...
		catch (Exception ex)
		{
			if (operation != null && !operation.SpawnContractInstalled)
			{
				try
				{
					if (operation.OwnedSpawnPoints != null && SameNativeSpawnList(GameManager.SpawnPointsInScene, operation.OwnedSpawnPoints))
					{
					...
					GameManager val6 = GameManager.instance;
					if ((Object)(object)val6 != (Object)null && operation.OwnedFallbackSpawns != null && SameNativeGameObjectArray(val6.Pspawns, operation.OwnedFallbackSpawns))
					{
						val6.Pspawns = (IsUsableSpawnArray(operation.PreviousFallbackSpawns) ? operation.PreviousFallbackSpawns : new Il2CppReferenceArray<GameObject>(0L));
						val6.PnextSpawnIndex = Math.Max(0, operation.PreviousNextSpawnIndex);
					}
					if ((Object)(object)val6 != (Object)null && operation.RandomSpawnsCaptured && val6.RandomSpawns == operation.OwnedRandomSpawns)
					{
						val6.RandomSpawns = operation.PreviousRandomSpawns;
					}
				}
				catch
				{
			...
			error = ex.GetType().Name + ": " + ex.Message;
			log.LogError((object)("Standalone package player spawn contract failed: " + error));
			return false;
		}
	}
	...
	private static bool ValidateStandaloneSceneContract(Scene scene, ActiveMapOperation operation, out string error)
	{
		...
		if ((int)operation.Operation.Mode == 1)
		{
			int num = list.Count((Transform marker) => !IsTeamTwoPlayerMarker(((Object)marker).name));
			int num2 = list.Count((Transform marker) => IsTeamTwoPlayerMarker(((Object)marker).name));
			int num3 = Math.Max(1, (operation.Operation.MaximumPlayers + 1) / 2);
			if (num < num3 || num2 < num3)
			{
				error = "PVP mode requires at least " + num3 + " spawn markers per team for maximumPlayers=" + operation.Operation.MaximumPlayers + "; found Team1=" + num + ", Team2=" + num2;
				return false;
			}
		}
		bool flag = false;
	...
	private bool TryClaimStandalonePvpAllPlayersLoaded(StandalonePvpGameMode gameMode)
	{
		//IL_0021: Unknown result type (might be due to invalid IL or missing references)
		//IL_0027: Invalid comparison between Unknown and I4
		ActiveMapOperation activeMapOperation = activeOperation;
		if (activeMapOperation == null || (Object)(object)gameMode == (Object)null || activeMapOperation.Operation == null || (int)activeMapOperation.Operation.Mode != 1 || (Object)(object)activeMapOperation.GameModeComponent != (Object)(object)gameMode || activeMapOperation.AllPlayersLoadedClaimed || activeMapOperation.NetworkSpawnFailed)
		{
			return false;
		}
		activeMapOperation.AllPlayersLoadedClaimed = true;
		return true;
	}
	...
	private bool TryAdoptNetworkSpawnedGameMode(ActiveMapOperation operation, GameMode gameMode)
	{
		...
		if ((int)operation.Operation.Mode == 1 && !IsPvpNetworkSpawnExpected(operation))
		{
			return false;
		}
	...
	private void MarkStandaloneReadinessInitialized(GameMode gameMode, string source)
	{
		...
		if (activeMapOperation != null && !((Object)(object)activeMapOperation.GameModeComponent != (Object)(object)gameMode))
		{
			...
			NotifyPvpNativeReadinessInitialized(activeMapOperation);
	...
	private void MarkStandalonePvpReadinessInitializationFailed(StandalonePvpGameMode gameMode, string source, Exception exception)
	{
		//IL_0026: Unknown result type (might be due to invalid IL or missing references)
		//IL_002c: Invalid comparison between Unknown and I4
		ActiveMapOperation activeMapOperation = activeOperation;
		if (activeMapOperation != null && !((Object)(object)activeMapOperation.GameModeComponent != (Object)(object)gameMode) && activeMapOperation.Operation != null && (int)activeMapOperation.Operation.Mode == 1)
		{
			activeMapOperation.ReadinessInitialized = false;
			string text = "PvpGameode.OnStartClient failed on its single native attempt: source=" + source + ", " + exception.GetType().Name + ": " + exception.Message;
			log.LogError((object)("Standalone PVP readiness failed closed: " + text));
			FailActivePvpNativeLifecycle(activeMapOperation, text);
		}
	}
	...
	private void OnStandaloneAllPlayersLoaded(bool nativePvpLifecycle)
	{
		...
		if (activeMapOperation != null)
		{
			...
			NotifyPvpAllPlayersLoaded(activeMapOperation);
	...
	private void OnStandalonePvpAllPlayersLoadedFailed(StandalonePvpGameMode gameMode, Exception exception)
	{
		//IL_0026: Unknown result type (might be due to invalid IL or missing references)
		//IL_002c: Invalid comparison between Unknown and I4
		ActiveMapOperation activeMapOperation = activeOperation;
		...
		{
			activeMapOperation.NativePvpLifecycleActive = false;
			string text = "PvpGameode.Server_AllPlayersLoaded failed on its single native attempt: " + exception.GetType().Name + ": " + exception.Message;
			log.LogError((object)("Standalone PVP all-players-loaded lifecycle failed closed: " + text));
			FailActivePvpNativeLifecycle(activeMapOperation, text);
		}
	}
	...
	private void MaintainStandaloneGameplay()
	{
		...
		if ((NetworkServer.active || (!NetworkServer.active && !NetworkClient.active)) && (int)activeMapOperation.Operation.Mode == 1 && activeMapOperation.NetworkSpawnFailed && !activeMapOperation.PvpAbortReturnRequested)
		{
			bool networkSpawnRequested = activeMapOperation.NetworkSpawnRequested;
			RequestNativePvpAbortReturn(activeMapOperation);
			ReleaseStandaloneSceneContracts(activeMapOperation);
			activeMapOperation.ScenePreparationComplete = false;
			activeMapOperation.ScenePreparationStarted = true;
			activeMapOperation.NetworkSpawnFailed = true;
			log.LogError((object)("Standalone PVP generation was torn down after its peer agreement failed: previouslySpawned=" + networkSpawnRequested + "."));
			return;
		}
		...
		if (!activeMapOperation.ScenePreparationComplete || !activeMapOperation.TerrainReady || (Object)(object)activeMapOperation.BootstrapRoot == (Object)null)
		{
			return;
		}
		ProcessPendingStandalonePveTeamValidation(activeMapOperation);
		if (Time.frameCount < activeMapOperation.LastMaintenanceFrame + 15)
		{
			return;
		}
		...
		{
			bool num = (int)activeMapOperation.Operation.Mode == 1;
			if (num)
			{
				activeMapOperation.NetworkSpawnFailed = true;
			}
			...
			if (num)
			{
				FailActivePvpNativeLifecycle(activeMapOperation, "Mirror prefab registration failed: " + ex.GetType().Name + ": " + ex.Message);
			}
			return;
		}
		if (NetworkServer.active && !activeMapOperation.NetworkSpawnRequested && !activeMapOperation.NetworkSpawnFailed && IsPvpNetworkSpawnAuthorized(activeMapOperation) && (Object)(object)activeMapOperation.BootstrapIdentity != (Object)null && activeMapOperation.BootstrapAssetId != 0)
		{
			if (!ValidateStandaloneBootstrapSyncObjects(activeMapOperation.BootstrapRoot, out var error))
			{
				...
				if ((int)activeMapOperation.Operation.Mode == 1)
				{
					FailActivePvpNativeLifecycle(activeMapOperation, "Mirror sync-object validation failed before spawn: " + error);
				}
				return;
			}
			try
			{
				...
				NotifyPvpNetworkOwnerSpawned(activeMapOperation);
			...
			catch (Exception ex2)
			{
				...
				if ((int)activeMapOperation.Operation.Mode == 1)
				{
					FailActivePvpNativeLifecycle(activeMapOperation, "NetworkServer.Spawn failed on its single native attempt: " + ex2.GetType().Name + ": " + ex2.Message);
				}
			}
		}
		if (!NetworkServer.active)
	...
	private static void ReleaseStandaloneSceneContracts(ActiveMapOperation operation)
	{
		if (operation != null)
		{
			DestroyOwnedStandalonePvePopulation(operation, "standalone scene-contract release");
			RestoreStandalonePlayerSpawnContract(operation);
	...
	private static void ReleaseStandaloneGameMode(ActiveMapOperation operation)
	{
		...
		if ((Object)(object)bootstrapPrefabRoot != (Object)null && ((Object)(object)bootstrapRoot == (Object)null || ((Object)bootstrapPrefabRoot).GetInstanceID() != ((Object)bootstrapRoot).GetInstanceID()))
		{
			Object.Destroy((Object)(object)bootstrapPrefabRoot);
		}
	...
	private void TrySpawnStandalonePveEnemies(ActiveMapOperation operation)
	{
		...
		GameManager val = GameManager.instance;
		if (!TryResolveLocalPlayerTeamId(out var teamId, out var source))
		{
			operation.PveSpawnAttempted = false;
			log.LogInfo((object)"Standalone PVE is waiting for the local player's native NetworkTeamID before hostile cohort selection.");
			return;
		}
		...
		if (!TrySelectStandalonePvePrefabCohort(val, teamId, out var selected, out var firearmCapablePrefabs, out var cohortSummary, out var failure))
		{
			log.LogError((object)("Standalone PVE hostile prefab selection failed closed: " + failure + ", playerTeam=" + teamId + ", playerTeamSource=" + source + ", cohorts=" + cohortSummary + "."));
			return;
		}
		List<GameObject> prefabs = selected.Prefabs;
		operation.PvePlayerTeamId = teamId;
		operation.PveExpectedTeamId = selected.TeamId;
		operation.PveExpectedStartingTeamStats = selected.StartingTeamStats;
		operation.PveExpectedStartingTeamStatsInstanceId = selected.StartingTeamStatsInstanceId;
		operation.PveExpectedTeamName = selected.TeamName;
		...
		try
		{
			...
			int num = CountRegisteredStandalonePveBrains(val);
			if (num != 0)
			{
				throw new InvalidOperationException("native GameManager.allAI baseline is not empty before the one-shot population call: rawEntries=" + num);
			}
			log.LogInfo((object)"Standalone PVE confirmed an empty native GameManager.allAI raw-entry baseline before its one-shot population call: rawEntries=0.");
			RaidManager.singleton = pveRaidManager;
			int val2 = ChooseStandalonePveEnemyCount(operation);
			int num2 = Math.Min(val2, list.Count);
			GameMode gameModeComponent = operation.GameModeComponent;
			...
			val.botAmount = num2;
			val.botHVTAmount = 0;
			HashSet<uint> preexistingServerNetIds = CaptureServerSpawnedNetIds();
			Il2CppReferenceArray<GameObject> standardAI = pveRaidManager.standardAI;
			...
			operation.PveExpectedSpawnCount = num2;
			try
			{
				pveRaidManager.standardAI = new Il2CppReferenceArray<GameObject>((long)prefabs.Count);
				for (int i = 0; i < prefabs.Count; i++)
				{
					((Il2CppArrayBase<GameObject>)(object)pveRaidManager.standardAI)[i] = prefabs[i];
				}
			...
			finally
			{
				try
				{
					pveRaidManager.standardAI = standardAI;
				}
				finally
				{
					CaptureOwnedStandalonePveServerPopulation(operation, preexistingServerNetIds);
				}
			}
			pveRaidManager.exfilZones = new List<ExfilZone>();
			pveRaidManager.exfilZones.Add(operation.PveExfilZone);
			if (operation.PveOwnedServerIdentities.Count != num2 || operation.PveOwnedBrainInstanceIds.Count != num2)
			{
				throw new InvalidOperationException("native Mirror ownership capture failed: ownedRoots=" + operation.PveOwnedServerIdentities.Count + ", ownedBrains=" + operation.PveOwnedBrainInstanceIds.Count + ", expected=" + num2);
			}
			operation.PveTeamValidationPending = true;
			SuppressStandalonePveExtraction(operation);
			log.LogInfo((object)("Standalone PVE issued an exact server-owned AI population through shipped RaidManager.ServerSpawnAI; native BrainAI.Start validation is pending: count=" + num2 + ", requestedRange=" + minimumEnemies + "-" + maximumEnemies + ", chosen=" + val2 + ", markers=" + list.Count + ", firearmCapablePrefabs=" + firearmCapablePrefabs + ", selectedCohortPrefabs=" + prefabs.Count + ", playerTeam=" + teamId + ", hostileTeam=" + selected.TeamId + ", hostileTeamName=" + selected.TeamName + ", hostileTeamStatsInstance=" + selected.StartingTeamStatsInstanceId + ", ownedNetIds=" + operation.PveOwnedServerIdentities.Count + ", issuedFrame=" + operation.PveTeamValidationIssuedFrame + ", earliestFrame=" + operation.PveTeamValidationEarliestFrame + ", deadlineFrame=" + operation.PveTeamValidationDeadlineFrame + ", raidExfilsAfterNativeSpawn=" + pveRaidManager.exfilZones.Count + ", aiProfile=" + FormatPveAiProfile(pveAiProfile) + "."));
		}
		catch (Exception ex)
		{
			int count = operation.PveOwnedServerIdentities.Count;
			...
			log.LogError((object)("Standalone PVE spawn failed closed after capturing " + count + " exact server roots: " + ex.GetType().Name + ": " + ex.Message));
		}
	}

	private static bool TryResolveLocalPlayerTeamId(out int teamId, out string source)
	{
		teamId = -1;
		source = "unresolved";
		try
		{
			PlayerNetworking myPlayerNetworking = GameManager.myPlayerNetworking;
			object identifier;
			if (myPlayerNetworking == null)
			{
				identifier = null;
			}
			else
			{
				TeamIdentifierReference myTeamIdentifierReference = myPlayerNetworking.MyTeamIdentifierReference;
				identifier = ((myTeamIdentifierReference != null) ? myTeamIdentifierReference.MyTeamIdentifier : null);
			}
			if (TryReadResolvedPlayerTeamId((TeamIdentifier)identifier, (myPlayerNetworking != null) ? myPlayerNetworking.MyTeamIdentifierReference : null, out teamId))
			{
			...
			{
				PlayerMaster playerMaster = myPlayerNetworking.playerMaster;
				identifier2 = ((playerMaster != null) ? playerMaster.MyTeamIdentifier : null);
			}
			if (TryReadResolvedPlayerTeamId((TeamIdentifier)identifier2, null, out teamId))
			{
				source = "GameManager.myPlayerNetworking.playerMaster";
				return true;
			}
			GameObject myPlayer = GameManager.myPlayer;
			TeamIdentifierReference val = (((Object)(object)myPlayer == (Object)null) ? null : myPlayer.GetComponentInParent<TeamIdentifierReference>());
			if (TryReadResolvedPlayerTeamId(((val != null) ? val.MyTeamIdentifier : null) ?? (((Object)(object)myPlayer == (Object)null) ? null : myPlayer.GetComponentInParent<TeamIdentifier>()), val, out teamId))
			{
	...
	private static bool TryReadResolvedPlayerTeamId(TeamIdentifier identifier, TeamIdentifierReference teamReference, out int teamId)
	{
		teamId = -1;
		if ((Object)(object)identifier == (Object)null || (Object)(object)identifier.TeamStats == (Object)null || identifier.TargetPool == null)
		{
			return false;
		}
		int networkTeamID = identifier.NetworkTeamID;
		if (networkTeamID == -1 || identifier.TeamID != networkTeamID || identifier.TeamStats.ThisTeamId != networkTeamID || identifier.TargetPool.Team != networkTeamID)
		{
			return false;
		}
		if ((Object)(object)teamReference != (Object)null && ((Object)(object)teamReference.MyTeamIdentifier != (Object)(object)identifier || teamReference.TargetPool != identifier.TargetPool || teamReference.TeamId != networkTeamID))
		{
			return false;
		}
	...
	private bool TrySelectStandalonePvePrefabCohort(GameManager gameManager, int playerTeamId, out StandalonePvePrefabCohort selected, out int firearmCapablePrefabs, out string cohortSummary, out string failure)
	{
		selected = null;
		firearmCapablePrefabs = 0;
		cohortSummary = "none";
		failure = "GameManager.AllAITypes is unavailable";
		List<GameObject> val = (((Object)(object)gameManager == (Object)null) ? null : gameManager.AllAITypes);
		if (val == null)
		{
			return false;
		}
		Dictionary<string, StandalonePvePrefabCohort> dictionary = new Dictionary<string, StandalonePvePrefabCohort>(StringComparer.Ordinal);
		int num = 0;
		for (int i = 0; i < val.Count; i++)
		{
			GameObject val2 = val[i];
			try
			{
				BrainAI val3 = (((Object)(object)val2 == (Object)null) ? null : val2.GetComponent<BrainAI>());
				NetworkIdentity val4 = (((Object)(object)val2 == (Object)null) ? null : val2.GetComponent<NetworkIdentity>());
				WeaponsAI val5 = (((Object)(object)val2 == (Object)null) ? null : val2.GetComponent<WeaponsAI>());
				if ((Object)(object)val2 == (Object)null || (Object)(object)val3 == (Object)null || (Object)(object)val4 == (Object)null || (Object)(object)val5 == (Object)null || !val5.SpawnWeapon || val5.weaponList == null || val5.weaponList.Count == 0)
				{
					continue;
				}
				firearmCapablePrefabs++;
				TeamIdentifier component = val2.GetComponent<TeamIdentifier>();
				TeamIdentifierReference component2 = val2.GetComponent<TeamIdentifierReference>();
				TeamStats val6 = ((component != null) ? component.StartingTeamStats : null);
				int num2 = (((Object)(object)val6 == (Object)null) ? (-1) : val6.ThisTeamId);
				if ((Object)(object)component == (Object)null || (Object)(object)component2 == (Object)null || (Object)(object)val6 == (Object)null || num2 == -1)
				{
					num++;
				}
				else
				{
					if (num2 == playerTeamId)
					{
						continue;
					}
					int instanceID = ((Object)val6).GetInstanceID();
					string key = instanceID.ToString(CultureInfo.InvariantCulture) + ":" + num2.ToString(CultureInfo.InvariantCulture);
					if (!dictionary.TryGetValue(key, out var value))
					{
						string teamName;
						try
						{
							teamName = val6.TeamName ?? "unnamed";
						}
						catch
						{
							teamName = "unreadable";
						}
						value = new StandalonePvePrefabCohort
						{
							StartingTeamStats = val6,
							StartingTeamStatsInstanceId = instanceID,
							TeamId = num2,
							TeamName = teamName
						};
						dictionary.Add(key, value);
					}
					value.Prefabs.Add(val2);
					continue;
				}
			}
			catch
		...
		List<StandalonePvePrefabCohort> list = (from cohort in dictionary.Values
			orderby cohort.Prefabs.Count descending, cohort.TeamId, cohort.StartingTeamStatsInstanceId
			select cohort).ToList();
		cohortSummary = ((list.Count == 0) ? "none" : string.Join(",", list.Select((StandalonePvePrefabCohort cohort) => cohort.TeamName + "#" + cohort.TeamId + "@" + cohort.StartingTeamStatsInstanceId + "=" + cohort.Prefabs.Count)));
		if (list.Count == 0)
		{
			failure = "no firearm-capable hostile prefab satisfies the native TeamIdentifier contract (firearmCapable=" + firearmCapablePrefabs + ", rejectedTeamContract=" + num + ")";
			return false;
		}
		if (list.Count > 1 && list[0].Prefabs.Count == list[1].Prefabs.Count)
		{
			failure = "hostile prefab cohorts have no unique strict-largest member (firearmCapable=" + firearmCapablePrefabs + ", rejectedTeamContract=" + num + ")";
			return false;
		}
		selected = list[0];
		failure = string.Empty;
		log.LogInfo((object)("Standalone PVE selected one native hostile prefab cohort: playerTeam=" + playerTeamId + ", selected=" + selected.TeamName + "#" + selected.TeamId + "@" + selected.StartingTeamStatsInstanceId + ", prefabs=" + selected.Prefabs.Count + ", firearmCapable=" + firearmCapablePrefabs + ", rejectedTeamContract=" + num + ", cohorts=" + cohortSummary + "."));
		return true;
	}
	...
	private static HashSet<uint> CaptureServerSpawnedNetIds()
	{
		...
		{
			throw new InvalidOperationException("Mirror NetworkServer.spawned is unavailable before native AI spawn");
		}
		HashSet<uint> hashSet = new HashSet<uint>();
		Enumerator<uint, NetworkIdentity> enumerator = NetworkServer.spawned.GetEnumerator();
		try
		{
			while (enumerator.MoveNext())
			{
				hashSet.Add(enumerator.Current.Key);
			}
			return hashSet;
		}
		finally
	...
	private static int CountRegisteredStandalonePveBrains(GameManager gameManager)
	{
		if ((Object)(object)gameManager == (Object)null || gameManager.allAI == null)
		{
			throw new InvalidOperationException("native GameManager.allAI registry is unavailable before AI spawn");
		}
		return gameManager.allAI.Count;
	}

	private static void CaptureOwnedStandalonePveServerPopulation(ActiveMapOperation operation, HashSet<uint> preexistingServerNetIds)
	{
		if (operation == null || preexistingServerNetIds == null || NetworkServer.spawned == null)
		{
			throw new InvalidOperationException("Mirror ownership capture is unavailable after native AI spawn");
		}
		...
		try
		{
			while (enumerator.MoveNext())
			{
				KeyValuePair<uint, NetworkIdentity> current = enumerator.Current;
				uint key = current.Key;
				NetworkIdentity value = current.Value;
				if (preexistingServerNetIds.Contains(key))
				{
					continue;
				}
				operation.PveOwnedServerIdentities.Add(key, value);
				if (key != 0 && !((Object)(object)value == (Object)null) && value.netId == key)
				{
					GameObject gameObject = ((Component)value).gameObject;
					BrainAI val = (((Object)(object)gameObject == (Object)null) ? null : gameObject.GetComponent<BrainAI>());
					if (!((Object)(object)gameObject == (Object)null) && !((Object)(object)val == (Object)null) && !((Object)(object)((Component)value).transform.root != (Object)(object)((Component)value).transform) && !((Object)(object)((Component)val).transform != (Object)(object)((Component)value).transform))
					{
						operation.PveOwnedBrainInstanceIds.Add(((Object)val).GetInstanceID());
					}
				}
			}
		}
	...
	private void ProcessPendingStandalonePveTeamValidation(ActiveMapOperation operation)
	{
		if (operation == null)
		{
			return;
		...
		{
			if (!operation.PveTeamValidationPending || operation.PveTeamContractValidated)
			{
				return;
			}
			SuppressStandalonePveExtraction(operation);
			...
			if (!NetworkServer.active)
			{
				FailStandalonePveTeamValidation(operation, "Mirror server stopped before native BrainAI startup completed");
				return;
			}
			GameManager val = GameManager.instance;
			List<BrainAI> ownedStandalonePveBrains = GetOwnedStandalonePveBrains(operation, val);
			int pveExpectedSpawnCount = operation.PveExpectedSpawnCount;
			if (!TryValidateOwnedStandalonePveServerPopulation(operation, out var summary))
			{
				FailStandalonePveTeamValidation(operation, "exact Mirror ownership vanished or changed before native BrainAI startup completed: " + summary);
				return;
			}
			if (ownedStandalonePveBrains.Count > pveExpectedSpawnCount)
			{
				FailStandalonePveTeamValidation(operation, "registered exact-owned BrainAI count exceeded the declared population: registeredOwnedBrains=" + ownedStandalonePveBrains.Count + ", expected=" + pveExpectedSpawnCount);
				return;
			}
			if (ownedStandalonePveBrains.Count < pveExpectedSpawnCount)
			{
				string text = "registeredOwnedBrains=" + ownedStandalonePveBrains.Count + ", ownedNetIds=" + operation.PveOwnedServerIdentities.Count + ", expected=" + pveExpectedSpawnCount + ", issuedFrame=" + operation.PveTeamValidationIssuedFrame + ", currentFrame=" + Time.frameCount + ", deadlineFrame=" + operation.PveTeamValidationDeadlineFrame;
				if (Time.frameCount < operation.PveTeamValidationDeadlineFrame)
				{
					log.LogInfo((object)("Standalone PVE is waiting for deferred native BrainAI.Start registration: " + text + "."));
				}
				else
				{
					FailStandalonePveTeamValidation(operation, text);
				}
				return;
			}
			int num;
			try
			{
				num = ((((val != null) ? val.allAI : null) == null) ? (-1) : val.allAI.Count);
			}
			catch
			{
				num = -1;
			}
			if (num != pveExpectedSpawnCount)
			{
				FailStandalonePveTeamValidation(operation, "native GameManager.allAI raw count does not equal the exact owned population after startup: rawEntries=" + num + ", ownedBrains=" + ownedStandalonePveBrains.Count + ", expected=" + pveExpectedSpawnCount);
				return;
			}
			if (!TryValidateStandalonePveSpawnedTeamContract(operation, val, pveExpectedSpawnCount, out var summary2))
			{
				FailStandalonePveTeamValidation(operation, "native spawned-team contract failed: " + summary2);
				return;
			}
			operation.PveTeamValidationPending = false;
			operation.PveTeamContractValidated = true;
			operation.PveEnemyCount = pveExpectedSpawnCount;
			StartProfiledPveAiDiagnostics(operation, val);
			log.LogInfo((object)("Standalone PVE released its exact server-owned AI population after deferred native startup validation: count=" + pveExpectedSpawnCount + ", validationFrame=" + Time.frameCount + ", issuedFrame=" + operation.PveTeamValidationIssuedFrame + ", rawAllAI=" + num + ", teamContract=" + summary2 + "."));
		}
	}

	private void FailStandalonePveTeamValidation(ActiveMapOperation operation, string summary)
	{
		if (operation != null)
		{
			operation.PveTeamValidationPending = false;
			...
			operation.PveTeamContractFailed = true;
			SuppressStandalonePveExtraction(operation);
			log.LogError((object)("Standalone PVE validation failed closed: " + summary + "."));
			DestroyOwnedStandalonePvePopulation(operation, "native team-contract failure");
		}
	}

	private static void SuppressStandalonePveExtraction(ActiveMapOperation operation)
	{
		if (!NetworkServer.active)
		{
			return;
		...
		try
		{
			ExfilZone val = operation?.PveExfilZone;
			if ((Object)(object)val != (Object)null)
			{
				if (val._occupants == null)
				{
					val._occupants = new HashSet<int>();
				}
				else
				{
					val._occupants.Clear();
				}
				val.NetworkPlayersInExfil = 0;
				val.PlayersInExfil = 0;
				val.NetworkcanExtract = false;
				val.canExtract = false;
			}
			GameManagerNetwork val2 = GameManagerNetwork.instance;
			if ((Object)(object)val2 != (Object)null)
			{
				if (val2._globalExfilOccupants == null)
				{
					val2._globalExfilOccupants = new HashSet<int>();
				}
				else
				{
					val2._globalExfilOccupants.Clear();
				}
				val2.NetworkPlayersInAnyExfil = 0;
				val2.PlayersInAnyExfil = 0;
				val2.NetworkcanExtract = false;
				val2.canExtract = false;
				val2.NetworkisExtracting = false;
				val2.isExtracting = false;
				val2.NetworkextractionStartTime = 0.0;
				val2.extractionStartTime = 0.0;
			}
		}
		catch
		{
	...
	private static bool TryValidateOwnedStandalonePveServerPopulation(ActiveMapOperation operation, out string summary)
	{
		int num = 0;
		int num2 = 0;
		int num3 = 0;
		int num4 = 0;
		if (operation == null || !NetworkServer.active || NetworkServer.spawned == null)
		{
			summary = "Mirror server registry unavailable";
			return false;
		}
		NetworkIdentity val = default(NetworkIdentity);
		foreach (KeyValuePair<uint, NetworkIdentity> pveOwnedServerIdentity in operation.PveOwnedServerIdentities)
		{
			uint key = pveOwnedServerIdentity.Key;
			NetworkIdentity value = pveOwnedServerIdentity.Value;
			if (!NetworkServer.spawned.TryGetValue(key, ref val) || (Object)(object)val == (Object)null)
			{
				num2++;
				continue;
			}
			if ((Object)(object)value == (Object)null || (Object)(object)val != (Object)(object)value || val.netId != key || (Object)(object)((Component)val).transform.root != (Object)(object)((Component)val).transform)
			{
				num3++;
				continue;
			}
			BrainAI component = ((Component)val).gameObject.GetComponent<BrainAI>();
			if ((Object)(object)component == (Object)null || !operation.PveOwnedBrainInstanceIds.Contains(((Object)component).GetInstanceID()) || (Object)(object)((Component)component).transform.root != (Object)(object)((Component)val).transform)
			{
				num4++;
			}
			else
		...
		bool result = num == operation.PveOwnedServerIdentities.Count && num == operation.PveOwnedBrainInstanceIds.Count && num2 == 0 && num3 == 0 && num4 == 0;
		summary = "valid=" + num + "/" + operation.PveOwnedServerIdentities.Count + ", ownedBrainIds=" + operation.PveOwnedBrainInstanceIds.Count + ", vanished=" + num2 + ", identityMismatches=" + num3 + ", brainMismatches=" + num4;
		return result;
	}

	private bool TryValidateStandalonePveSpawnedTeamContract(ActiveMapOperation operation, GameManager gameManager, int expectedCount, out string summary)
	{
		List<BrainAI> ownedStandalonePveBrains = GetOwnedStandalonePveBrains(operation, gameManager);
		if (ownedStandalonePveBrains.Count != expectedCount)
		{
			summary = "spawnedBrainCount=" + ownedStandalonePveBrains.Count + ", expected=" + expectedCount;
			return false;
		}
		int num = 0;
		int num2 = 0;
		...
		int num6 = 0;
		int num7 = 0;
		HashSet<int> hashSet = new HashSet<int>(from brain in ownedStandalonePveBrains
			where (Object)(object)brain != (Object)null
			select ((Object)brain).GetInstanceID());
		TeamManager val = TeamManager.instance;
		bool flag = false;
		try
		{
			flag = (Object)(object)val != (Object)null && val.AllTeams != null && operation.PveExpectedTeamId >= 0 && operation.PveExpectedTeamId < ((Il2CppArrayBase<TeamStats>)(object)val.AllTeams).Length && (Object)(object)((Il2CppArrayBase<TeamStats>)(object)val.AllTeams)[operation.PveExpectedTeamId] == (Object)(object)operation.PveExpectedStartingTeamStats;
		}
		catch
		{
			flag = false;
		}
		foreach (BrainAI item in ownedStandalonePveBrains)
		{
			if ((Object)(object)item == (Object)null)
			{
				num2++;
			...
			try
			{
				TeamIdentifier component = ((Component)item).gameObject.GetComponent<TeamIdentifier>();
				TeamIdentifierReference component2 = ((Component)item).gameObject.GetComponent<TeamIdentifierReference>();
				if ((Object)(object)component == (Object)null || (Object)(object)component2 == (Object)null || (Object)(object)component.StartingTeamStats != (Object)(object)operation.PveExpectedStartingTeamStats || ((Object)component.StartingTeamStats).GetInstanceID() != operation.PveExpectedStartingTeamStatsInstanceId || (Object)(object)component2.MyTeamIdentifier != (Object)(object)component || (Object)(object)item.MyTeamIdentifierReference != (Object)(object)component2 || component.TargetPool == null || component2.TargetPool != component.TargetPool)
				{
					num2++;
					continue;
				}
				if (component.NetworkTeamID == -1)
				{
					component.NetworkTeamID = operation.PveExpectedTeamId;
					num++;
				}
				if (component.NetworkTeamID != operation.PveExpectedTeamId || component.TeamID != operation.PveExpectedTeamId || component.TargetPool.Team != operation.PveExpectedTeamId || component2.TeamId != operation.PveExpectedTeamId)
				{
					num3++;
				}
				if ((Object)(object)component.TeamStats != (Object)(object)operation.PveExpectedStartingTeamStats)
				{
					num4++;
				}
				if (item.allFriendlyBrains != null)
				{
					for (int num8 = 0; num8 < item.allFriendlyBrains.Count; num8++)
					{
						BrainAI val2 = item.allFriendlyBrains[num8];
				...
				if (item.allEnemyBrains != null)
				{
					for (int num9 = 0; num9 < item.allEnemyBrains.Count; num9++)
					{
						BrainAI val3 = item.allEnemyBrains[num9];
						if ((Object)(object)val3 != (Object)null && hashSet.Contains(((Object)val3).GetInstanceID()))
						{
							num5++;
						}
					}
				}
				if (item.possibleTargets == null)
				{
					continue;
				}
				for (int num10 = 0; num10 < item.possibleTargets.Count; num10++)
				{
					if (IsTargetOwnedByStandalonePveCohort(item.possibleTargets[num10], ownedStandalonePveBrains))
					{
						num6++;
					}
				}
		...
		bool result = flag && num2 == 0 && num3 == 0 && num4 == 0 && num5 == 0 && num6 == 0;
		summary = "team=" + operation.PveExpectedTeamId + ", teamName=" + operation.PveExpectedTeamName + ", teamStatsInstance=" + operation.PveExpectedStartingTeamStatsInstanceId + ", playerTeam=" + operation.PvePlayerTeamId + ", brains=" + ownedStandalonePveBrains.Count + "/" + expectedCount + ", managerTeamMatches=" + flag + ", repairedUnresolved=" + num + ", identifierFailures=" + num2 + ", teamStateFailures=" + num3 + ", teamStatsFailures=" + num4 + ", friendlyRelationships=" + num7 + ", cohortInEnemyLists=" + num5 + ", cohortInPossibleTargets=" + num6;
		return result;
	}

	private static List<BrainAI> GetOwnedStandalonePveBrains(ActiveMapOperation operation, GameManager gameManager)
	{
		List<BrainAI> list = new List<BrainAI>();
		if (operation == null || operation.PveOwnedBrainInstanceIds.Count == 0 || ((gameManager != null) ? gameManager.allAI : null) == null)
		{
			return list;
		}
		...
		for (int i = 0; i < gameManager.allAI.Count; i++)
		{
			...
			int instanceID = ((Object)val).GetInstanceID();
			if (!operation.PveOwnedBrainInstanceIds.Contains(instanceID) || !hashSet.Add(instanceID))
			{
				continue;
			}
			Transform root = ((Component)val).transform.root;
			if (!((Object)(object)root == (Object)null) && !((Object)(object)((Component)val).transform != (Object)(object)root))
			{
				NetworkIdentity component = ((Component)val).gameObject.GetComponent<NetworkIdentity>();
				if (!((Object)(object)component == (Object)null) && !((Object)(object)((Component)component).transform != (Object)(object)root) && component.netId != 0 && operation.PveOwnedServerIdentities.TryGetValue(component.netId, out var value) && !((Object)(object)value == (Object)null) && !((Object)(object)value != (Object)(object)component) && NetworkServer.spawned != null && NetworkServer.spawned.TryGetValue(component.netId, ref val2) && !((Object)(object)val2 == (Object)null) && !((Object)(object)val2 != (Object)(object)component) && !((Object)(object)((Component)val2).gameObject != (Object)(object)((Component)val).gameObject) && !((Object)(object)((Component)val2).gameObject.GetComponent<BrainAI>() != (Object)(object)val))
				{
					list.Add(val);
				}
			}
	...
	private static bool IsTargetOwnedByStandalonePveCohort(Transform target, IEnumerable<BrainAI> cohort)
	{
		...
		foreach (BrainAI item in cohort)
		{
			if ((Object)(object)item != (Object)null && (Object)(object)((Component)item).transform.root == (Object)(object)root)
			{
				return true;
			}
		}
	...
	private static BrainAI FindBrainOwningTarget(Transform target, GameManager gameManager)
	{
		if ((Object)(object)target == (Object)null || ((gameManager != null) ? gameManager.allAI : null) == null)
		{
			return null;
		}
		Transform root = target.root;
	...
	private static bool TryGetBrainNetworkTeamId(BrainAI brain, out int teamId)
	{
		teamId = -1;
		try
		{
			...
			{
				TeamIdentifierReference myTeamIdentifierReference = brain.MyTeamIdentifierReference;
				obj = ((myTeamIdentifierReference != null) ? myTeamIdentifierReference.MyTeamIdentifier : null);
			}
			TeamIdentifier val = (TeamIdentifier)obj;
			if ((Object)(object)val == (Object)null)
			{
				return false;
			}
			teamId = val.NetworkTeamID;
			return teamId != -1;
		}
		catch
		{
	...
	private static void DestroyOwnedStandalonePvePopulation(ActiveMapOperation operation, string reason)
	{
		if (operation == null)
		{
			return;
		...
		int num = 0;
		int num2 = 0;
		KeyValuePair<uint, NetworkIdentity>[] array = operation.PveOwnedServerIdentities.ToArray();
		NetworkIdentity val2 = default(NetworkIdentity);
		for (int i = 0; i < array.Length; i++)
		{
			KeyValuePair<uint, NetworkIdentity> keyValuePair = array[i];
			uint key = keyValuePair.Key;
			NetworkIdentity value = keyValuePair.Value;
			GameObject val = (((Object)(object)value == (Object)null) ? null : ((Component)value).gameObject);
			if ((Object)(object)val == (Object)null)
			{
				continue;
			}
			...
			{
				bool flag = false;
				if (NetworkServer.active && NetworkServer.spawned != null && NetworkServer.spawned.TryGetValue(key, ref val2))
				{
					flag = (Object)(object)val2 != (Object)null && (Object)(object)val2 == (Object)(object)value && (Object)(object)((Component)val2).gameObject == (Object)(object)val;
				}
				if (flag)
				{
			...
			catch (Exception ex)
			{
				...
				if (cerberusNativeTabFix != null)
				{
					...
					if (obj != null)
					{
						obj.LogWarning((object)("Standalone PVE exact root cleanup skipped netId=" + key + ": " + ex.GetType().Name + ": " + ex.Message));
					}
				}
			}
		}
		if (count > 0)
		{
			CerberusNativeTabFix cerberusNativeTabFix2 = instance;
			if (cerberusNativeTabFix2 != null)
			{
				ManualLogSource obj2 = cerberusNativeTabFix2.log;
				if (obj2 != null)
				{
					obj2.LogWarning((object)("Standalone PVE removed its exact server-owned population: roots=" + count + ", networkDestroyed=" + num + ", locallyDestroyed=" + num2 + ", reason=" + reason + "."));
				}
			}
		}
		operation.PveOwnedServerIdentities.Clear();
	...
	private static void ResetProfiledPveGenerationState(ActiveMapOperation operation)
	{
		if (operation != null)
		{
			operation.PveOwnedServerIdentities.Clear();
	...
	private bool TryBeginProfiledPveAiDiagnostics(ActiveMapOperation operation, GameManager gameManager, string source)
	{
		...
		{
			if (IsProfiledPveBrainLive(profiledPveDiagnosticBrain))
			{
		...
		for (int i = 0; i < gameManager.allAI.Count; i++)
		{
			...
			if (!((Object)(object)val == (Object)null))
			{
				...
				if (operation.PveOwnedBrainInstanceIds.Contains(instanceID) && hashSet.Add(instanceID))
				{
					...
					TryApplyProfiledPveMaximumReactionTimeCap(operation, val);
	...
	private static bool IsProfiledPveBrainLive(BrainAI brain)
	{
		try
		{
			AgentController val = ((brain != null) ? brain.agent : null);
			return (Object)(object)brain != (Object)null && ((Behaviour)brain).enabled && ((Component)brain).gameObject.activeInHierarchy && (Object)(object)val != (Object)null && ((Behaviour)val).enabled && ((Behaviour)val).isActiveAndEnabled && val.entityExists;
		}
		catch
		{
			return false;
	...
	private void TryApplyProfiledPveMaximumReactionTimeCap(ActiveMapOperation operation, BrainAI brain)
	{
		float? obj;
		if (operation == null)
		{
		...
		else
		{
			...
			else
			{
				...
				obj = ((pveAiProfile != null) ? pveAiProfile.MaximumReactionTimeSeconds : ((float?)null));
			}
		}
		float? num = obj;
		...
		try
		{
			...
			float baseReactionTime = brain._baseReactionTime;
			float reactionTime = brain.ReactionTime;
			if (!float.IsFinite(value) || value <= 0f)
			{
				operation.ProfiledPveReactionTimeSkipped++;
				return;
			}
			if (!float.IsFinite(baseReactionTime) || baseReactionTime < 0f || !float.IsFinite(reactionTime) || reactionTime < 0f)
			{
				operation.ProfiledPveReactionTimeSkipped++;
				return;
			}
			float num2 = Mathf.Min(baseReactionTime, value);
			float num3 = Mathf.Min(reactionTime, value);
			bool num4 = !Mathf.Approximately(baseReactionTime, num2) || !Mathf.Approximately(reactionTime, num3);
			brain._baseReactionTime = num2;
			brain.ReactionTime = num3;
			if (num4)
			{
		...
		{
			operation.ProfiledPveReactionTimeSkipped++;
			log.LogWarning((object)("Profiled PVE maximum reaction-time cap skipped one new native BrainAI: operation=" + operation.Operation.Id + ", brain=" + instanceID + ", " + ex.GetType().Name + ": " + ex.Message));
		}
	}
	...
	private void LogProfiledPveNativeAiContract(ActiveMapOperation operation, string source)
	{
		...
		foreach (BrainAI profiledPveDiagnosticBrain in operation.ProfiledPveDiagnosticBrains)
		{
			...
			try
			{
				float num17 = (float)profiledPveDiagnosticBrain.WanderTimer * profiledPveDiagnosticBrain.Patience;
				num2 = Mathf.Min(num2, num17);
				num3 = Mathf.Max(num3, num17);
				float num18 = Mathf.Max(0f, num17 - profiledPveDiagnosticBrain.wanderTime);
				num4 = Mathf.Min(num4, num18);
				num5 = Mathf.Max(num5, num18);
				...
				num10 = Mathf.Min(num10, profiledPveDiagnosticBrain._baseReactionTime);
				num11 = Mathf.Max(num11, profiledPveDiagnosticBrain._baseReactionTime);
				num12 = Mathf.Min(num12, profiledPveDiagnosticBrain.ReactionTime);
				num13 = Mathf.Max(num13, profiledPveDiagnosticBrain.ReactionTime);
				if ((int)profiledPveDiagnosticBrain.idleStates == 1)
				{
					num14++;
				}
				if ((int)profiledPveDiagnosticBrain.reactType == 1)
				{
					num16++;
				}
		...
		float? maximumReactionTimeSeconds = operation.Operation.PveAiProfile.MaximumReactionTimeSeconds;
		if (maximumReactionTimeSeconds.HasValue)
		{
			log.LogInfo((object)("Profiled PVE maximum reaction-time cap: operation=" + operation.Operation.Id + ", profile=" + operation.Operation.PveAiProfile.Id + ", cap=" + maximumReactionTimeSeconds.Value.ToString("F3", CultureInfo.InvariantCulture) + "s, handled=" + operation.ProfiledPveReactionTimeHandledBrainIds.Count + "/" + num + ", capped=" + operation.ProfiledPveReactionTimeCapped + ", preserved=" + operation.ProfiledPveReactionTimePreserved + ", skipped=" + operation.ProfiledPveReactionTimeSkipped + ", observedBase=" + num10.ToString("F3", CultureInfo.InvariantCulture) + ".." + num11.ToString("F3", CultureInfo.InvariantCulture) + "s, observedCurrent=" + num12.ToString("F3", CultureInfo.InvariantCulture) + ".." + num13.ToString("F3", CultureInfo.InvariantCulture) + "s, reactionTimerUnchanged=True, nativeDifficultyUnchanged=True."));
		}
	}
	...
	private void LogProfiledPveAiSnapshot(ActiveMapOperation operation, GameManager gameManager, float scheduled, float elapsed)
	{
		...
		float num37 = float.MinValue;
		float num38 = float.MaxValue;
		float num39 = float.MinValue;
		float num40 = 0f;
		float num41 = 0f;
		float num42 = 0f;
		float num43 = 0f;
		...
		foreach (BrainAI profiledPveDiagnosticBrain in operation.ProfiledPveDiagnosticBrains)
		{
			...
			try
			{
				...
				num42 += magnitude;
				num43 = Mathf.Max(num43, magnitude);
				if (magnitude >= 1f)
				{
					num3++;
				}
				...
				Transform currentSeenTarget = profiledPveDiagnosticBrain.CurrentSeenTarget;
				if ((Object)(object)currentSeenTarget != (Object)null)
				{
					num5++;
					if ((Object)(object)val != (Object)null && (Object)(object)currentSeenTarget.root == (Object)(object)val.transform.root)
					{
						num6++;
					}
					else
					{
						BrainAI val6 = FindBrainOwningTarget(currentSeenTarget, gameManager);
						int teamId;
						int teamId2;
						if ((Object)(object)val6 == (Object)null)
						{
						...
						else if (TryGetBrainNetworkTeamId(profiledPveDiagnosticBrain, out teamId) && TryGetBrainNetworkTeamId(val6, out teamId2))
						{
							if (teamId == teamId2)
							{
								num7++;
				...
				num36 = Mathf.Min(num36, profiledPveDiagnosticBrain.wanderTime);
				num37 = Mathf.Max(num37, profiledPveDiagnosticBrain.wanderTime);
				...
				if (profiledPveDiagnosticBrain.allFriendlyBrains != null)
				{
					for (int i = 0; i < profiledPveDiagnosticBrain.allFriendlyBrains.Count; i++)
					{
						BrainAI candidate = profiledPveDiagnosticBrain.allFriendlyBrains[i];
						if ((Object)(object)candidate != (Object)null && operation.ProfiledPveDiagnosticBrains.Any((BrainAI member) => (Object)(object)member != (Object)null && ((Object)member).GetInstanceID() == ((Object)candidate).GetInstanceID()))
						{
							num12++;
						}
					}
				}
				if (profiledPveDiagnosticBrain.allEnemyBrains != null)
				{
					for (int num44 = 0; num44 < profiledPveDiagnosticBrain.allEnemyBrains.Count; num44++)
					{
						BrainAI candidate2 = profiledPveDiagnosticBrain.allEnemyBrains[num44];
						if ((Object)(object)candidate2 != (Object)null && operation.ProfiledPveDiagnosticBrains.Any((BrainAI member) => (Object)(object)member != (Object)null && ((Object)member).GetInstanceID() == ((Object)candidate2).GetInstanceID()))
						{
							num10++;
						}
					}
				}
				if (profiledPveDiagnosticBrain.possibleTargets != null)
				{
					for (int num45 = 0; num45 < profiledPveDiagnosticBrain.possibleTargets.Count; num45++)
					{
						if (IsTargetOwnedByStandalonePveCohort(profiledPveDiagnosticBrain.possibleTargets[num45], operation.ProfiledPveDiagnosticBrains))
						{
							num11++;
						}
					}
				...
				num38 = Mathf.Min(num38, maxSpeed);
				num39 = Mathf.Max(num39, maxSpeed);
				Vector3 velocity = agent.velocity;
				float magnitude2 = ((Vector3)(ref velocity)).magnitude;
				num40 = Mathf.Max(num40, magnitude2);
				if (magnitude2 >= 0.01f)
				{
					num33++;
				}
				Vector3 position = agent.position;
				num41 = Mathf.Max(num41, Vector3.Distance(position, profiledPveNavigationPosition));
				Vector3 destination = agent.destination;
				((Vector2)(ref val7))._002Ector(destination.x - profiledPveNavigationPosition.x, destination.z - profiledPveNavigationPosition.z);
				...
				if (Physics.Linecast(((Object)(object)eyesTransform == (Object)null) ? (profiledPveNavigationPosition + Vector3.up * 1.6f) : eyesTransform.transform.position, val2, ref val8, LayerMask.op_Implicit(profiledPveDiagnosticBrain.eyesAI.DetectionLayerMask), (QueryTriggerInteraction)2))
				{
					Transform val9 = (((Object)(object)((RaycastHit)(ref val8)).collider == (Object)null) ? null : ((Component)((RaycastHit)(ref val8)).collider).transform);
					if ((Object)(object)val9 != (Object)null && (Object)(object)val9.root == (Object)(object)val.transform.root)
					{
						num15++;
					}
					else if ((Object)(object)((RaycastHit)(ref val8)).collider != (Object)null && ((Component)((RaycastHit)(ref val8)).collider).gameObject.layer == 18)
					{
						num13++;
					}
					else
		...
		float num46 = ((num2 == 0) ? 0f : (num42 / (float)num2));
		log.LogInfo((object)("Profiled PVE AI snapshot: operation=" + operation.Operation.Id + ", profile=" + operation.Operation.PveAiProfile.Id + ", scheduled=" + scheduled.ToString("F0", CultureInfo.InvariantCulture) + "s, elapsed=" + elapsed.ToString("F2", CultureInfo.InvariantCulture) + "s, tracked=" + num + ", live=" + num2 + ", moved>=1m=" + num3 + ", movedTowardInsertion>=5m=" + num4 + ", movementMean=" + num46.ToString("F2", CultureInfo.InvariantCulture) + "m, movementMax=" + num43.ToString("F2", CultureInfo.InvariantCulture) + "m, actualSeenTarget=" + num5 + "(player=" + num6 + ",friendlyBrain=" + num7 + ",enemyBrain=" + num8 + ",other=" + num9 + "), targetGraph(friendlyCohortRefs=" + num12 + ",enemyCohortRefs=" + num10 + ",possibleCohortTargets=" + num11 + "), sameMaskSightProbe(vegetation=" + num13 + ",other=" + num14 + ",clearOrPlayer=" + num15 + "), states=" + text + "."));
		string text2 = ((dictionary2.Count == 0) ? "none" : string.Join(",", from pair in dictionary2
			orderby pair.Key
			select pair.Key + "=" + pair.Value));
		string text3 = ((num2 == 0) ? "none" : (num36.ToString("F2", CultureInfo.InvariantCulture) + ".." + num37.ToString("F2", CultureInfo.InvariantCulture) + "s"));
		string text4 = ((num20 == 0) ? "none" : (num38.ToString("F2", CultureInfo.InvariantCulture) + ".." + num39.ToString("F2", CultureInfo.InvariantCulture) + "mps"));
		log.LogInfo((object)("Profiled PVE native agent diagnostic: operation=" + operation.Operation.Id + ", scheduled=" + scheduled.ToString("F0", CultureInfo.InvariantCulture) + "s, tracked=" + num + ", live=" + num2 + ", brainEnabled=" + num16 + "/" + num + ", controllerAssigned=" + num17 + "/" + num + ", controllerEnabled=" + num18 + "/" + num + ", controllerActive=" + num19 + "/" + num + ", entityExists=" + num20 + "/" + num + ", updatePosition=" + num21 + "/" + num2 + ", pathPending=" + num22 + ", hasPath=" + num23 + ", destination>=1m=" + num24 + ", followerAssigned=" + num25 + "/" + num2 + ", followerEnabled=" + num26 + "/" + num2 + ", followerActive=" + num27 + "/" + num2 + ", followerEntityExists=" + num28 + "/" + num2 + ", canMove=" + num29 + "/" + num2 + ", canSearch=" + num30 + "/" + num2 + ", simulateMovement=" + num31 + "/" + num2 + ", isStopped=" + num32 + "/" + num2 + ", maxSpeed=" + text4 + ", velocity>=0.01mps=" + num33 + ", velocityMax=" + num40.ToString("F2", CultureInfo.InvariantCulture) + "mps, controllerPositionOffsetMax=" + num41.ToString("F2", CultureInfo.InvariantCulture) + "m, responding=" + num34 + ", currentCover=" + num35 + ", wanderClock=" + text3 + ", movementTypes=" + text2 + "."));
	}
	...
	private static void ConfigureStandaloneBotDetails(BotSpawnDetails details, ModdedPveAiProfileDefinition profile)
	{
		...
		if (profile != null && profile.ReactionDisposition != null)
		{
			switch (profile.ReactionDisposition)
			{
			case "defensive":
				details.reactType = (ReactType)0;
				break;
			case "offensive":
				details.reactType = (ReactType)1;
				break;
			case "random":
				details.reactType = (ReactType)2;
				break;
			default:
				throw new InvalidOperationException("validated PVE AI profile has an unsupported reactionDisposition");
			}
		}
	...
	private static List<Transform> FindStandalonePlayerMarkers(Scene scene, ModdedOperationMode mode)
	{
		...
		foreach (GameObject item in (Il2CppArrayBase<GameObject>)(object)((Scene)(ref scene)).GetRootGameObjects())
		{
			foreach (Transform componentsInChild in item.GetComponentsInChildren<Transform>(true))
			{
				...
				bool flag = text.StartsWith("Team1_Spawn_", StringComparison.OrdinalIgnoreCase) || text.StartsWith("Team1_Backup_Spawn_", StringComparison.OrdinalIgnoreCase);
				if ((int)mode == 2 && (flag || text.StartsWith("PVE_PlayerSpawn_", StringComparison.OrdinalIgnoreCase)))
				{
					list.Add(componentsInChild);
				}
				else if ((int)mode == 1 && (flag || text.StartsWith("PVP_Team1Spawn_", StringComparison.OrdinalIgnoreCase)))
				{
					list.Add(componentsInChild);
				}
				else if ((int)mode == 1 && (text.StartsWith("Team2_Spawn_", StringComparison.OrdinalIgnoreCase) || text.StartsWith("Team2_Backup_Spawn_", StringComparison.OrdinalIgnoreCase) || text.StartsWith("PVP_Team2Spawn_", StringComparison.OrdinalIgnoreCase)))
				{
	...
	private void MaintainPvpPeerAgreementTransport()
	{
		try
		{
			...
			{
				if (pvpClientHandlerRegistered || pvpServerHandlerRegistered || hostPvpAgreement != null || remotePvpAgreement != null)
				{
					ReleasePvpPeerAgreementTransport("network lifetime ended");
				}
				return;
			}
			NetworkMessageDelegate val = null;
			if (pvpClientHandlerRegistered && (NetworkClient.handlers == null || !NetworkClient.handlers.TryGetValue((ushort)19791, ref val) || !SamePvpAgreementHandler(val, pvpClientHandler)))
			{
				pvpClientHandlerRegistered = false;
				if ((Delegate)(object)val != (Delegate)null)
				{
					pvpAgreementTransportFailure = "Mirror client message ID ownership changed at 0x" + ((ushort)19791).ToString("X4", CultureInfo.InvariantCulture);
				}
			}
			NetworkMessageDelegate val2 = null;
			if (pvpServerHandlerRegistered && (NetworkServer.handlers == null || !NetworkServer.handlers.TryGetValue((ushort)19791, ref val2) || !SamePvpAgreementHandler(val2, pvpServerHandler)))
			{
				pvpServerHandlerRegistered = false;
				if ((Delegate)(object)val2 != (Delegate)null)
				{
					pvpAgreementTransportFailure = "Mirror server message ID ownership changed at 0x" + ((ushort)19791).ToString("X4", CultureInfo.InvariantCulture);
				}
			}
			if (NetworkClient.active && !pvpClientHandlerRegistered)
			{
				...
				{
					throw new InvalidOperationException("Mirror client handler registry is unavailable");
				}
				NetworkMessageDelegate val3 = default(NetworkMessageDelegate);
				if (NetworkClient.handlers.TryGetValue((ushort)19791, ref val3))
				{
					pvpAgreementTransportFailure = "Mirror client message ID collision at 0x" + ((ushort)19791).ToString("X4", CultureInfo.InvariantCulture);
				}
				else
				{
					pvpClientManagedHandler = HandlePvpClientAgreementEnvelope;
					pvpClientHandler = NetworkMessageDelegate.op_Implicit(pvpClientManagedHandler);
					NetworkClient.handlers.Add((ushort)19791, pvpClientHandler);
			...
			if (NetworkServer.active && !pvpServerHandlerRegistered)
			{
				...
				{
					throw new InvalidOperationException("Mirror server handler registry is unavailable");
				}
				NetworkMessageDelegate val4 = default(NetworkMessageDelegate);
				if (NetworkServer.handlers.TryGetValue((ushort)19791, ref val4))
				{
					pvpAgreementTransportFailure = "Mirror server message ID collision at 0x" + ((ushort)19791).ToString("X4", CultureInfo.InvariantCulture);
				}
				else
				{
					pvpServerManagedHandler = HandlePvpServerAgreementEnvelope;
					pvpServerHandler = NetworkMessageDelegate.op_Implicit(pvpServerManagedHandler);
					NetworkServer.handlers.Add((ushort)19791, pvpServerHandler);
					pvpServerHandlerRegistered = true;
				}
			}
			if ((!string.IsNullOrEmpty(pvpAgreementTransportFailure) || (!pvpClientHandlerRegistered && !pvpServerHandlerRegistered)) && !string.IsNullOrEmpty(pvpAgreementTransportFailure))
			{
				FailHostPvpAgreement(pvpAgreementTransportFailure, sendCancel: true);
				FailRemotePvpAgreement(pvpAgreementTransportFailure, sendReject: true);
			}
		}
		catch (Exception ex)
		{
			pvpAgreementTransportFailure = ex.GetType().Name + ": " + ex.Message;
			FailHostPvpAgreement("PVP peer-agreement transport failed: " + pvpAgreementTransportFailure, sendCancel: true);
			FailRemotePvpAgreement("PVP peer-agreement transport failed: " + pvpAgreementTransportFailure, sendReject: true);
		}
	}
	...
	private void ReleasePvpPeerAgreementTransport(string reason)
	{
		...
		if (activeMapOperation != null)
		{
			...
			if ((int)((operation != null) ? new ModdedOperationMode?(operation.Mode) : ((ModdedOperationMode?)null)).GetValueOrDefault() == 1)
			{
				if (hostPvpAgreement == null || !OperationMatchesPvpIdentity(activeMapOperation, hostPvpAgreement.Identity))
				{
					if (remotePvpAgreement != null)
					{
						num = (OperationMatchesPvpIdentity(activeMapOperation, remotePvpAgreement.Identity) ? 1 : 0);
						if (num != 0)
						{
							goto IL_0074;
						}
		...
		try
		{
			...
			else if (remotePvpAgreement != null && flag)
			{
				SendRemotePvpControl(PvpAgreementMessageKind.Reject, reason, null, 0uL);
			}
		}
		catch
		{
		...
		try
		{
			NetworkMessageDelegate left = default(NetworkMessageDelegate);
			if (pvpClientHandlerRegistered && NetworkClient.handlers != null && NetworkClient.handlers.TryGetValue((ushort)19791, ref left) && SamePvpAgreementHandler(left, pvpClientHandler))
			{
				NetworkClient.handlers.Remove((ushort)19791);
			}
		}
		catch
		{
		...
		try
		{
			NetworkMessageDelegate left2 = default(NetworkMessageDelegate);
			if (pvpServerHandlerRegistered && NetworkServer.handlers != null && NetworkServer.handlers.TryGetValue((ushort)19791, ref left2) && SamePvpAgreementHandler(left2, pvpServerHandler))
			{
				NetworkServer.handlers.Remove((ushort)19791);
			}
		}
		catch
		{
		...
		goto IL_00b0;
		IL_0074:
		if (!activeMapOperation.NativeLaunchInvoked && activeMapOperation.SceneHandle == 0)
		{
			HostPvpAgreement obj4 = hostPvpAgreement;
			if (obj4 == null || !obj4.NativeLaunchInvoked)
			{
				num2 = ((remotePvpAgreement?.ContentCommitted ?? false) ? 1 : 0);
				goto IL_00b0;
			}
		}
		num2 = 1;
	...
	private static bool SamePvpAgreementHandler(NetworkMessageDelegate left, NetworkMessageDelegate right)
	{
		if (left == right)
		{
			return true;
		...
		try
		{
			return (Delegate)(object)left != (Delegate)null && (Delegate)(object)right != (Delegate)null && ((Il2CppObjectBase)left).Pointer == ((Il2CppObjectBase)right).Pointer;
		}
		catch
		{
			return false;
	...
	private void BeginPvpPeerAgreementOrFail(CatalogPresentation presentation, ModdedMapDefinition map, ModdedOperationDefinition operation, string timeCode, MissionLaptop launchLaptop, PlayerNetworking launchPlayer, SceneVariantSelection sceneSelection)
	{
		//IL_0004: Unknown result type (might be due to invalid IL or missing references)
		//IL_000a: Invalid comparison between Unknown and I4
		try
		{
			if (operation == null || (int)operation.Mode != 1)
			{
				throw new InvalidOperationException("peer agreement can only start for a PVP operation");
			}
			if (!NetworkServer.active || !NetworkClient.active)
			{
				throw new InvalidOperationException("PVP package launch requires an active native multiplayer host");
			}
			MaintainPvpPeerAgreementTransport();
			if (!pvpClientHandlerRegistered || !pvpServerHandlerRegistered || !string.IsNullOrEmpty(pvpAgreementTransportFailure))
			{
				throw new InvalidOperationException(pvpAgreementTransportFailure ?? "PVP peer-agreement handlers are not available");
			}
			if (!SelectionBelongsToMap(map, sceneSelection))
			{
				throw new InvalidOperationException("selected PVP scene variant is not owned by the selected map");
			}
			if (activeOperation != null && activeOperation.SceneHandle != 0)
			{
				throw new InvalidOperationException("another package scene is still active on the host");
			}
			if (this.hostPvpAgreement != null)
			{
				BroadcastHostPvpControl(PvpAgreementMessageKind.Cancel, "a new host selection replaced the prior PVP agreement");
			}
			this.hostPvpAgreement = null;
			Dictionary<int, NetworkConnectionToClient> dictionary = CaptureAuthenticatedRemotePvpConnections(requireReady: true);
			int num = dictionary.Count + 1;
			if (dictionary.Count == 0)
			{
				throw new InvalidOperationException("PVP package launch requires at least one authenticated remote peer");
			}
			if (num < operation.MinimumPlayers || num > operation.MaximumPlayers)
			{
				throw new InvalidOperationException("PVP lobby population " + num + " is outside the operation range " + operation.MinimumPlayers + "-" + operation.MaximumPlayers);
			}
			PvpPeerIdentity pvpPeerIdentity = CreatePvpPeerIdentity(map, operation, timeCode, sceneSelection, Guid.NewGuid().ToString("N"));
			HostPvpAgreement hostPvpAgreement = new HostPvpAgreement
			{
				Presentation = presentation,
				Map = map,
				Operation = operation,
				...
				SceneSelection = sceneSelection,
				Identity = pvpPeerIdentity,
				Phase = HostPvpAgreementPhase.WaitingForContent,
				DeadlineTimestamp = DeadlineAfter(45.0)
			};
			foreach (KeyValuePair<int, NetworkConnectionToClient> item in dictionary)
			{
				hostPvpAgreement.RequiredConnectionIds.Add(item.Key);
				hostPvpAgreement.RequiredConnections.Add(item.Key, item.Value);
			}
			this.hostPvpAgreement = hostPvpAgreement;
			foreach (KeyValuePair<int, NetworkConnectionToClient> item2 in dictionary)
			{
				SendPvpOffer((NetworkConnection)(object)item2.Value, pvpPeerIdentity);
			}
			log.LogInfo((object)("PVP exact-content agreement offered to " + hostPvpAgreement.RequiredConnectionIds.Count + " remote peer(s): package=" + pvpPeerIdentity.PackageId + "@" + pvpPeerIdentity.PackageVersion + ", content=" + pvpPeerIdentity.PackageContentId + ", operation=" + pvpPeerIdentity.OperationId + ", variant=" + pvpPeerIdentity.VariantId + "."));
			log.LogInfo((object)("PVP runtime binary agreement identity: frameworkSha256=" + pvpPeerIdentity.FrameworkSha256 + ", apiCoreSha256=" + pvpPeerIdentity.ApiCoreSha256 + ", apiHostSha256=" + pvpPeerIdentity.ApiHostSha256 + ", companion=" + pvpPeerIdentity.CompanionPluginGuid + "@" + pvpPeerIdentity.CompanionPluginVersion + ", companionSha256=" + pvpPeerIdentity.CompanionSha256 + "."));
		}
		catch (Exception ex)
		{
			SetNativeConfirmationLoadingState(presentation, loading: false);
			this.hostPvpAgreement = null;
			log.LogError((object)("PVP package launch failed closed before native start: " + ex.GetType().Name + ": " + ex.Message + "."));
		}
	}
	...
	private void ClearPvpPeerAgreementForNonPvpLaunch()
	{
		if (hostPvpAgreement != null)
		{
			try
			{
				BroadcastHostPvpControl(PvpAgreementMessageKind.Cancel, "a non-PVP launch closed the prior PVP agreement");
			}
			catch
			{
			}
	...
	private void ProcessPvpPeerAgreement()
	{
		...
		HostPvpAgreement hostPvpAgreement = this.hostPvpAgreement;
		if (hostPvpAgreement != null)
		{
			if (hostPvpAgreement.NativeLaunchInvoked && (activeOperation == null || !OperationMatchesPvpIdentity(activeOperation, hostPvpAgreement.Identity)))
			{
				FailHostPvpAgreement("active operation ownership changed after native PVP launch", sendCancel: true);
				return;
			}
			if (!TryValidateHostPvpMembership(hostPvpAgreement, out var error))
			{
				FailHostPvpAgreement(error, sendCancel: true);
				return;
			}
			if ((hostPvpAgreement.Phase == HostPvpAgreementPhase.WaitingForContent || hostPvpAgreement.Phase == HostPvpAgreementPhase.WaitingForScenes) && DeadlineExpired(hostPvpAgreement.DeadlineTimestamp))
			{
				string text = ((hostPvpAgreement.Phase == HostPvpAgreementPhase.WaitingForContent) ? "content-ready" : "scene-ready");
				FailHostPvpAgreement("timed out waiting for the exact remote " + text + " barrier", sendCancel: true);
				return;
			}
			if (hostPvpAgreement.Phase == HostPvpAgreementPhase.WaitingForScenes && hostPvpAgreement.SceneGenerationEpoch != 0L && DeadlineExpired(hostPvpAgreement.SceneReadyRequestRetryTimestamp))
			{
				try
				{
					BroadcastHostPvpSceneReadyRequest(hostPvpAgreement, isRetry: true);
				}
				catch (Exception ex)
				{
					FailHostPvpAgreement("host scene-generation request retry failed: " + ex.GetType().Name + ": " + ex.Message, sendCancel: true);
					return;
				}
			}
			if (hostPvpAgreement.Phase == HostPvpAgreementPhase.ReadyToSpawn && DeadlineExpired(hostPvpAgreement.DeadlineTimestamp))
			{
				FailHostPvpAgreement("timed out waiting to publish the exact native PVP owner", sendCancel: true);
				return;
			}
			if (hostPvpAgreement.Phase == HostPvpAgreementPhase.WaitingForContent && hostPvpAgreement.RequiredConnectionIds.SetEquals(hostPvpAgreement.ContentReadyConnectionIds))
			{
				if (!TryBeginHostPvpSceneGeneration(hostPvpAgreement, "initial native scene transition", out var error2))
				{
					FailHostPvpAgreement(error2, sendCancel: true);
					return;
				}
				log.LogInfo((object)("PVP exact-content agreement passed on every peer; entering the native scene transition for operation=" + hostPvpAgreement.Operation.Id + "."));
				log.LogInfo((object)("PVP initial scene-generation readiness is bound to epoch=" + hostPvpAgreement.SceneGenerationEpoch + "."));
				InvokeNativeCatalogLaunch(hostPvpAgreement.Presentation, hostPvpAgreement.Map, hostPvpAgreement.Operation, hostPvpAgreement.TimeCode, hostPvpAgreement.LaunchLaptop, hostPvpAgreement.LaunchPlayer, hostPvpAgreement.SceneSelection, pvpPeerAgreementSatisfied: true);
				return;
			}
			bool flag = activeOperation != null && OperationMatchesPvpIdentity(activeOperation, hostPvpAgreement.Identity) && activeOperation.AllPlayersLoaded;
			if (hostPvpAgreement.NativeLifecycleDeadlineTimestamp > 0 && DeadlineExpired(hostPvpAgreement.NativeLifecycleDeadlineTimestamp) && !flag)
			{
				FailHostPvpAgreement("timed out waiting for the exact native PVP lifecycle barrier", sendCancel: true);
				return;
			}
			if (hostPvpAgreement.Phase == HostPvpAgreementPhase.WaitingForScenes && IsHostLocalSceneReadyForCurrentEpoch(hostPvpAgreement) && AreRequiredPvpPeersSceneReadyForCurrentEpoch(hostPvpAgreement))
			{
				hostPvpAgreement.Phase = HostPvpAgreementPhase.ReadyToSpawn;
				hostPvpAgreement.DeadlineTimestamp = DeadlineAfter(90.0);
				log.LogInfo((object)"PVP scene-ready agreement passed on the host and all remote peers; native PvpGameode spawn is authorized.");
				log.LogInfo((object)("PVP scene-ready epoch barrier passed: sceneGenerationEpoch=" + hostPvpAgreement.SceneGenerationEpoch + "."));
			}
		}
		RemotePvpAgreement remotePvpAgreement = this.remotePvpAgreement;
		if (remotePvpAgreement != null && remotePvpAgreement.ContentCommitted && (activeOperation == null || !OperationMatchesPvpIdentity(activeOperation, remotePvpAgreement.Identity)))
		{
			FailRemotePvpAgreement("active operation ownership changed after remote content commit", sendReject: true);
		}
		else if (remotePvpAgreement != null && DeadlineExpired(remotePvpAgreement.DeadlineTimestamp) && (!IsRemoteSceneReadyForCurrentRequest(remotePvpAgreement) || !remotePvpAgreement.NativeOwnerAdopted || !remotePvpAgreement.NativeReadinessInitialized))
		{
			FailRemotePvpAgreement((!remotePvpAgreement.ContentCommitted) ? "timed out preloading the agreed PVP package content" : ((!IsRemoteSceneReadyForCurrentRequest(remotePvpAgreement)) ? "timed out waiting for the agreed PVP scene to become ready" : ((!remotePvpAgreement.NativeOwnerAdopted) ? "timed out waiting for the agreed native PVP owner" : "timed out waiting for the agreed native PVP readiness lifecycle")), sendReject: true);
		}
	}

	private void HandlePvpClientAgreementEnvelope(NetworkConnection connection, NetworkReader reader, int channelId)
	{
		try
		{
			if (NetworkServer.active)
			...
			{
				throw new InvalidOperationException("PVP agreement arrived before client authentication");
			}
			ValidatePvpEnvelopeReader(reader);
			ushort num = NetworkReaderExtensions.ReadUShort(reader);
			PvpAgreementMessageKind pvpAgreementMessageKind = (PvpAgreementMessageKind)NetworkReaderExtensions.ReadByte(reader);
			if (num != 2)
			{
				throw new InvalidOperationException("unsupported PVP agreement protocol " + num);
			}
			if (pvpAgreementMessageKind == PvpAgreementMessageKind.Offer)
			{
			...
			string b = ReadBoundedPvpString(reader, 64, "nonce");
			string b2 = ReadBoundedPvpString(reader, 128, "digest");
			ulong num2 = NetworkReaderExtensions.ReadULong(reader);
			string reason = ReadBoundedPvpString(reader, 512, "reason", allowEmpty: true);
			RequirePvpEnvelopeConsumed(reader);
			RemotePvpAgreement remotePvpAgreement = this.remotePvpAgreement;
			bool flag = remotePvpAgreement != null && string.Equals(this.remotePvpAgreement.Identity.Nonce, b, StringComparison.Ordinal) && string.Equals(this.remotePvpAgreement.Identity.Digest, b2, StringComparison.Ordinal);
			switch (pvpAgreementMessageKind)
			{
			case PvpAgreementMessageKind.SceneReadyRequest:
				if (num2 == 0L)
				{
					throw new InvalidOperationException("host scene-ready request omitted its generation epoch");
				}
				if (flag)
				{
					AcceptRemotePvpSceneReadyRequest(remotePvpAgreement, num2);
				}
				break;
			case PvpAgreementMessageKind.Cancel:
				if (num2 != 0L)
				{
					throw new InvalidOperationException("host cancellation carried an invalid scene generation epoch");
				}
				if (flag)
				{
				...
				break;
			default:
				throw new InvalidOperationException("unexpected host PVP agreement control " + pvpAgreementMessageKind);
			}
		}
		catch (Exception ex)
		{
			FailRemotePvpAgreement("invalid host PVP agreement envelope: " + ex.Message, sendReject: true);
		}
	}

	private void HandlePvpServerAgreementEnvelope(NetworkConnection connection, NetworkReader reader, int channelId)
	{
		try
		{
			if (!NetworkServer.active || connection == null || !connection.isAuthenticated)
			{
				return;
			}
			ValidatePvpEnvelopeReader(reader);
			...
			string b2 = ReadBoundedPvpString(reader, 128, "digest");
			ulong num2 = NetworkReaderExtensions.ReadULong(reader);
			string text = ReadBoundedPvpString(reader, 512, "reason", allowEmpty: true);
			RequirePvpEnvelopeConsumed(reader);
			HostPvpAgreement hostPvpAgreement = this.hostPvpAgreement;
			if (hostPvpAgreement == null || !hostPvpAgreement.RequiredConnectionIds.Contains(connection.connectionId) || !hostPvpAgreement.RequiredConnections.TryGetValue(connection.connectionId, out var value) || !SamePvpNetworkConnection((NetworkConnection)(object)value, connection) || !string.Equals(hostPvpAgreement.Identity.Nonce, b, StringComparison.Ordinal) || !string.Equals(hostPvpAgreement.Identity.Digest, b2, StringComparison.Ordinal))
			{
				return;
			}
			switch (pvpAgreementMessageKind)
			{
			case PvpAgreementMessageKind.Reject:
				if (num2 != 0L)
				{
					throw new InvalidOperationException("remote rejection carried an invalid scene generation epoch");
				}
				FailHostPvpAgreement("remote peer " + connection.connectionId + " rejected the PVP agreement: " + text, sendCancel: true);
				return;
			case PvpAgreementMessageKind.ContentReady:
				if (num2 != 0L)
				{
					throw new InvalidOperationException("remote content-ready acknowledgement carried an invalid scene generation epoch");
				}
				break;
			}
			if (pvpAgreementMessageKind == PvpAgreementMessageKind.ContentReady && hostPvpAgreement.Phase == HostPvpAgreementPhase.WaitingForContent)
			{
				hostPvpAgreement.ContentReadyConnectionIds.Add(connection.connectionId);
				log.LogInfo((object)("PVP peer content-ready acknowledgement accepted: connection=" + connection.connectionId + ", ready=" + hostPvpAgreement.ContentReadyConnectionIds.Count + "/" + hostPvpAgreement.RequiredConnectionIds.Count + "."));
				return;
			}
			switch (pvpAgreementMessageKind)
			{
			case PvpAgreementMessageKind.SceneReady:
				if (num2 == 0L)
				{
					throw new InvalidOperationException("remote scene-ready acknowledgement omitted its generation epoch");
				}
				if (hostPvpAgreement.Phase == HostPvpAgreementPhase.WaitingForScenes && num2 == hostPvpAgreement.SceneGenerationEpoch)
				{
					hostPvpAgreement.SceneReadyEpochByConnectionId[connection.connectionId] = num2;
					log.LogInfo((object)("PVP peer scene-ready acknowledgement accepted: connection=" + connection.connectionId + ", ready=" + CountRequiredPvpPeersSceneReadyForCurrentEpoch(hostPvpAgreement) + "/" + hostPvpAgreement.RequiredConnectionIds.Count + "."));
					log.LogInfo((object)("PVP peer scene-ready epoch accepted: connection=" + connection.connectionId + ", sceneGenerationEpoch=" + num2 + "."));
				}
				else
				{
					log.LogWarning((object)("PVP peer scene-ready acknowledgement rejected as stale or out of phase: connection=" + connection.connectionId + ", receivedEpoch=" + num2 + ", expectedEpoch=" + hostPvpAgreement.SceneGenerationEpoch + ", phase=" + hostPvpAgreement.Phase.ToString() + "."));
					if (hostPvpAgreement.Phase == HostPvpAgreementPhase.WaitingForScenes && hostPvpAgreement.SceneGenerationEpoch != 0L)
					{
						SendHostPvpSceneReadyRequest(connection, hostPvpAgreement, isRetry: true);
					}
				}
				break;
			default:
				throw new InvalidOperationException("unexpected remote PVP agreement control " + pvpAgreementMessageKind);
			case PvpAgreementMessageKind.ContentReady:
				break;
			}
		}
		...
		{
			FailHostPvpAgreement("invalid remote PVP agreement envelope: " + ex.Message, sendCancel: true);
		}
	}
	...
	private void AcceptRemotePvpOfferOrReject(PvpPeerIdentity offer)
	{
		...
		if (activeOperation != null && activeOperation.SceneHandle != 0)
		{
			SendRemotePvpControl(PvpAgreementMessageKind.Reject, "another package scene is still active on the remote peer", offer, 0uL);
			return;
		}
		if (remotePvpAgreement != null)
		{
			if (string.Equals(remotePvpAgreement.Identity.Nonce, offer.Nonce, StringComparison.Ordinal) && string.Equals(remotePvpAgreement.Identity.Digest, offer.Digest, StringComparison.Ordinal))
			{
				if (remotePvpAgreement.ContentCommitted)
				{
					SendRemotePvpControl(PvpAgreementMessageKind.ContentReady, string.Empty, null, 0uL);
				}
				return;
			}
			try
			{
				SendRemotePvpControl(PvpAgreementMessageKind.Reject, "host replaced an unfinished PVP agreement without cancellation", offer, 0uL);
			}
			catch
			{
			}
			ClearRemotePvpAgreement("host replaced an unfinished PVP agreement without cancellation");
			log.LogError((object)"Remote PVP peer agreement failed closed: host replaced an unfinished PVP agreement without cancellation.");
			return;
		}
		if (!TryResolveExactLocalPvpOffer(offer, out var map, out var operation, out var selection, out var error))
		{
			SendRemotePvpControl(PvpAgreementMessageKind.Reject, error, offer, 0uL);
			log.LogError((object)("Remote PVP offer rejected before content loading: " + error + "."));
			return;
		}
		...
		if (loadedMapBundles.TryGetValue(map.Id, out var value) && value != null && (Object)(object)value.SceneBundle != (Object)null && value.Map != null && string.Equals(value.Map.PackageContentId, map.PackageContentId, StringComparison.Ordinal))
		{
			CompleteRemotePvpContentReady();
			return;
		}
		if (value != null && !TryDiscardStaleCompletedMapBundle(map.Id, value, "remote PVP exact-content offer"))
		{
			FailRemotePvpAgreement("stale map bundles could not be released for exact remote content", sendReject: true);
			return;
		}
		if (pendingLaunch != null)
		{
			if (pendingLaunch.LaunchRequested || pendingLaunch.Map == null || !string.Equals(pendingLaunch.Map.Id, map.Id, StringComparison.Ordinal) || !string.Equals(pendingLaunch.Map.PackageContentId, map.PackageContentId, StringComparison.Ordinal))
			{
				FailRemotePvpAgreement("another package load owns the remote content pipeline", sendReject: true);
			}
			return;
		}
		LoadedMapBundles loadingBundles = new LoadedMapBundles
		...
		foreach (string dependencyBundlePath in map.DependencyBundlePaths)
		{
			pendingMapLaunch.BundlePaths.Add(dependencyBundlePath);
		}
		pendingMapLaunch.BundlePaths.Add(map.SceneBundlePath);
		pendingLaunch = pendingMapLaunch;
		log.LogInfo((object)("Remote PVP peer began exact package preload: map=" + map.Id + ", package=" + map.PackageId + "@" + map.PackageVersion + ", content=" + map.PackageContentId + "."));
	}

	private bool TryResolveExactLocalPvpOffer(PvpPeerIdentity offer, out ModdedMapDefinition map, out ModdedOperationDefinition operation, out SceneVariantSelection selection, out string error)
	{
		...
		selection = null;
		error = string.Empty;
		if (!TryResolveLocalRuntimeIdentity(null, out var identity, out var error2))
		{
			error = "local runtime binary identity failed closed: " + error2;
			return false;
		}
		if (!string.Equals(offer.FrameworkVersion, "0.3.29", StringComparison.Ordinal) || !string.Equals(offer.FrameworkSha256, identity.FrameworkSha256, StringComparison.Ordinal) || !string.Equals(offer.ApiVersion, OperatorApi.ApiVersion, StringComparison.Ordinal) || !string.Equals(offer.ApiCoreSha256, identity.ApiCoreSha256, StringComparison.Ordinal) || !string.Equals(offer.ApiHostSha256, identity.ApiHostSha256, StringComparison.Ordinal) || !string.Equals(offer.GameBuildId, OperatorApi.Compatibility.DetectedGameBuildId, StringComparison.Ordinal) || !string.Equals(offer.Capabilities, "exact-content-v2;runtime-binary-sha256-v1;package-runtime-ready-v1;remote-preload-v1;scene-ready-epoch-v1;native-pvp-v1", StringComparison.Ordinal))
		{
			error = "framework/API binary/game-build/capability identity mismatch";
			return false;
		}
		if (!string.Equals(offer.Digest, ComputePvpIdentityDigest(offer), StringComparison.Ordinal))
		{
			error = "offer identity digest mismatch";
			return false;
		}
		if (!OperatorApi.ModdedOperations.TryGetMap(offer.MapId, ref map) || map == null)
		{
			error = "exact map ID is not present in the frozen local catalog";
			return false;
		}
		if (!string.Equals(map.PackageId, offer.PackageId, StringComparison.Ordinal) || !string.Equals(map.PackageVersion, offer.PackageVersion, StringComparison.Ordinal) || !string.Equals(map.PackageContentId, offer.PackageContentId, StringComparison.Ordinal))
		{
			error = "package ID/version/content hash mismatch";
			return false;
		}
		if (!TryResolveLocalRuntimeIdentity(map, out var identity2, out error2))
		{
			error = "local map runtime identity failed closed: " + error2;
			return false;
		}
		if (!PvpCompanionIdentityMatches(offer, identity2))
		{
			error = "map runtime companion GUID/version/SHA/marker identity mismatch";
			return false;
		}
		operation = map.Operations.FirstOrDefault((ModdedOperationDefinition candidate) => candidate != null && string.Equals(candidate.Id, offer.OperationId, StringComparison.Ordinal));
		if (operation == null || (int)operation.Mode != 1 || offer.Mode != 1 || !string.Equals(operation.PackageId, offer.PackageId, StringComparison.Ordinal) || !string.Equals(operation.SpawnSetId, offer.SpawnSetId, StringComparison.Ordinal) || operation.MinimumPlayers != offer.MinimumPlayers || operation.MaximumPlayers != offer.MaximumPlayers || !operation.SupportedTimeCodes.Contains<string>(offer.TimeCode, StringComparer.Ordinal))
		{
		...
		if (!HasDeclaredSceneVariants(map))
		{
			if (map.SceneVariants == null || map.SceneVariants.Count != 1 || !string.Equals(offer.VariantId, map.SceneVariants[0].Id, StringComparison.Ordinal))
			{
				goto IL_0310;
			}
			num = string.Equals(offer.ScenePath, map.ScenePath, StringComparison.OrdinalIgnoreCase);
		}
		else
		{
			num = map.SceneVariants.Any((ModdedSceneVariantDefinition variant) => variant != null && string.Equals(variant.Id, offer.VariantId, StringComparison.Ordinal) && string.Equals(variant.ScenePath, offer.ScenePath, StringComparison.OrdinalIgnoreCase));
		}
		if (num)
		{
			selection = new SceneVariantSelection(offer.VariantId, offer.ScenePath, 0);
	...
	{
		RemotePvpAgreement remotePvpAgreement = this.remotePvpAgreement;
		if (remotePvpAgreement != null && !remotePvpAgreement.ContentCommitted)
		{
			if (NetworkServer.active || !NetworkClient.active || NetworkClient.connection == null || !NetworkClient.connection.isAuthenticated)
			{
				FailRemotePvpAgreement("remote network ownership changed before exact content commit", sendReject: true);
				return;
			}
			if (!loadedMapBundles.TryGetValue(remotePvpAgreement.Map.Id, out var value) || value == null || (Object)(object)value.SceneBundle == (Object)null || value.Map == null || !string.Equals(value.Map.PackageContentId, remotePvpAgreement.Identity.PackageContentId, StringComparison.Ordinal))
			{
				FailRemotePvpAgreement("verified remote package cache was unavailable at commit", sendReject: true);
				return;
			}
			activeOperation = new ActiveMapOperation
			{
				Map = remotePvpAgreement.Map,
				Operation = remotePvpAgreement.Operation,
				TimeCode = remotePvpAgreement.Identity.TimeCode,
				SceneSelection = remotePvpAgreement.SceneSelection,
				SceneHandle = 0
			};
			remotePvpAgreement.ContentCommitted = true;
			remotePvpAgreement.DeadlineTimestamp = DeadlineAfter(135.0);
			SendRemotePvpControl(PvpAgreementMessageKind.ContentReady, string.Empty, null, 0uL);
			remotePvpAgreement.ContentReadySent = true;
			log.LogInfo((object)("Remote PVP peer committed the exact operation before native scene transition: operation=" + remotePvpAgreement.Operation.Id + ", scene=" + remotePvpAgreement.SceneSelection.ScenePath + "."));
		}
	}
	...
	{
		RemotePvpAgreement remotePvpAgreement = this.remotePvpAgreement;
		if (remotePvpAgreement != null && map != null && string.Equals(remotePvpAgreement.Map.Id, map.Id, StringComparison.Ordinal) && string.Equals(remotePvpAgreement.Identity.PackageContentId, map.PackageContentId, StringComparison.Ordinal))
		{
			CompleteRemotePvpContentReady();
		}
	}

	private void NotifyPvpPendingMapLoadFailed(PendingMapLaunch pending, string reason)
	{
		RemotePvpAgreement remotePvpAgreement = this.remotePvpAgreement;
		if (remotePvpAgreement != null && pending?.Map != null && string.Equals(remotePvpAgreement.Map.Id, pending.Map.Id, StringComparison.Ordinal) && string.Equals(remotePvpAgreement.Identity.PackageContentId, pending.Map.PackageContentId, StringComparison.Ordinal))
		{
			FailRemotePvpAgreement("exact remote package preload failed: " + reason, sendReject: true);
		}
	}

	private void NotifyPvpNativeLaunchFailed(Exception exception)
	{
		string reason = "native PVP launch failed: " + exception.GetType().Name + ": " + exception.Message;
		if (hostPvpAgreement != null)
		{
			FailHostPvpAgreement(reason, sendCancel: true);
		}
		else if (remotePvpAgreement != null)
		{
			FailRemotePvpAgreement(reason, sendReject: true);
		}
	}
	...
	private void NotifyPvpScenePrepared(ActiveMapOperation operation)
	{
		...
		if (operation != null && operation.Operation != null && (int)operation.Operation.Mode == 1)
		{
			if (operation.BootstrapAssetId != 1297043458 || !operation.BootstrapPrefabRegistered || !(operation.GameModeComponent is StandalonePvpGameMode))
			{
				NotifyPvpScenePreparationFailed(operation, "native PvpGameode template was not constructed and registered");
			}
			else
			{
	...
	private void TryAdvancePvpPackageRuntimeReadiness(ActiveMapOperation operation)
	{
		//IL_0011: Unknown result type (might be due to invalid IL or missing references)
		//IL_0017: Invalid comparison between Unknown and I4
		...
		//IL_01a7: Unknown result type (might be due to invalid IL or missing references)
		if (operation == null || operation.Operation == null || (int)operation.Operation.Mode != 1 || operation.NetworkSpawnFailed || !operation.ScenePreparationComplete || operation.SceneHandle == 0)
		{
			return;
		}
		HostPvpAgreement hostPvpAgreement = this.hostPvpAgreement;
		RemotePvpAgreement remotePvpAgreement = this.remotePvpAgreement;
		bool flag = NetworkServer.active && hostPvpAgreement != null && OperationMatchesPvpIdentity(operation, hostPvpAgreement.Identity);
		bool flag2 = !NetworkServer.active && remotePvpAgreement != null && remotePvpAgreement.ContentCommitted && OperationMatchesPvpIdentity(operation, remotePvpAgreement.Identity);
		if (!flag && !flag2)
		{
			return;
		}
		if (!TryValidateInstalledPvpSpawnContract(operation, out var error))
		{
			NotifyPvpScenePreparationFailed(operation, "package-owned PVP spawn contract changed before SceneReady: " + error);
			return;
		}
		ModdedRuntimeCompanionDefinition runtimeCompanion = operation.Map.RuntimeCompanion;
		Scene scene = FindLoadedSceneByHandle(operation.SceneHandle);
		if (!((Scene)(ref scene)).IsValid() || !((Scene)(ref scene)).isLoaded)
		{
			NotifyPvpScenePreparationFailed(operation, "exact generation scene was unavailable at runtime readiness gate");
			return;
		}
		if (runtimeCompanion != null)
		{
			FindExactSceneTransform(scene, runtimeCompanion.FailureMarkerName, out var matches);
			FindExactSceneTransform(scene, runtimeCompanion.ReadyMarkerName, out var matches2);
			if (matches != 0)
			{
				NotifyPvpScenePreparationFailed(operation, "package runtime companion emitted failure marker '" + runtimeCompanion.FailureMarkerName + "' in exact scene handle=" + ((object)((Scene)(ref scene)).handle/*cast due to constrained. prefix*/).ToString() + "; count=" + matches);
				return;
			}
			if ((flag && IsHostLocalSceneReadyForCurrentEpoch(hostPvpAgreement)) || (flag2 && remotePvpAgreement.LocalSceneReady && remotePvpAgreement.LocalReadySceneGeneration == remotePvpAgreement.LocalSceneGeneration))
			{
				return;
			}
			switch (matches2)
			...
			case 0:
				return;
			default:
				NotifyPvpScenePreparationFailed(operation, "package runtime companion ready marker was ambiguous in exact scene handle=" + ((object)((Scene)(ref scene)).handle/*cast due to constrained. prefix*/).ToString() + "; marker='" + runtimeCompanion.ReadyMarkerName + "', count=" + matches2);
				return;
			case 1:
				break;
			}
			log.LogInfo((object)("PVP package runtime readiness passed: companion=" + runtimeCompanion.PluginGuid + "@" + runtimeCompanion.PluginVersion + ", sceneHandle=" + ((object)((Scene)(ref scene)).handle/*cast due to constrained. prefix*/).ToString() + ", readyMarker=" + runtimeCompanion.ReadyMarkerName + "."));
		}
		else
		{
			if ((flag && IsHostLocalSceneReadyForCurrentEpoch(hostPvpAgreement)) || (flag2 && remotePvpAgreement.LocalSceneReady && remotePvpAgreement.LocalReadySceneGeneration == remotePvpAgreement.LocalSceneGeneration))
			...
			log.LogInfo((object)("PVP package runtime readiness passed: companion=none, sceneHandle=" + ((object)((Scene)(ref scene)).handle/*cast due to constrained. prefix*/).ToString() + "."));
		}
		CompletePvpSceneReady(operation);
	}

	private static bool TryValidateInstalledPvpSpawnContract(ActiveMapOperation operation, out string error)
	{
		error = string.Empty;
		GameManager val = GameManager.instance;
		if (operation == null || !operation.SpawnContractInstalled || operation.OwnedSpawnPoints == null || operation.OwnedFallbackSpawns == null || (Object)(object)val == (Object)null)
		{
			error = "installed spawn ownership is incomplete";
			return false;
		}
		if (!SameNativeSpawnList(GameManager.SpawnPointsInScene, operation.OwnedSpawnPoints) || !SameNativeGameObjectArray(val.Pspawns, operation.OwnedFallbackSpawns) || !operation.RandomSpawnsCaptured || operation.OwnedRandomSpawns || val.RandomSpawns != operation.OwnedRandomSpawns)
		{
	...
	{
		HostPvpAgreement hostPvpAgreement = this.hostPvpAgreement;
		if (NetworkServer.active && hostPvpAgreement != null && OperationMatchesPvpIdentity(operation, hostPvpAgreement.Identity))
		{
			if (hostPvpAgreement.SceneGenerationEpoch == 0L)
			{
				FailHostPvpAgreement("host scene became ready before a scene-generation epoch was issued", sendCancel: true);
				operation.NetworkSpawnFailed = true;
				return;
			}
			hostPvpAgreement.LocalSceneReady = true;
			hostPvpAgreement.LocalSceneReadyEpoch = hostPvpAgreement.SceneGenerationEpoch;
			log.LogInfo((object)("Host PVP scene preparation completed for sceneGenerationEpoch=" + hostPvpAgreement.SceneGenerationEpoch + "."));
		}
		RemotePvpAgreement remotePvpAgreement = this.remotePvpAgreement;
		if (!NetworkServer.active && remotePvpAgreement != null && remotePvpAgreement.ContentCommitted && OperationMatchesPvpIdentity(operation, remotePvpAgreement.Identity))
		{
			if (remotePvpAgreement.LocalSceneGeneration == 0L || remotePvpAgreement.LastObservedSceneHandle != operation.SceneHandle)
			{
				FailRemotePvpAgreement("remote scene readiness was not bound to an observed exact package scene generation", sendReject: true);
				operation.NetworkSpawnFailed = true;
			}
			else
			{
				remotePvpAgreement.LocalSceneReady = true;
				remotePvpAgreement.LocalReadySceneGeneration = remotePvpAgreement.LocalSceneGeneration;
				TrySendRemotePvpSceneReadyAcknowledgement(remotePvpAgreement, allowResend: false);
			}
		}
	}

	private bool TryBeginHostPvpSceneGeneration(HostPvpAgreement host, string reason, out string error)
	{
		error = string.Empty;
		...
		host.LocalSceneReadyEpoch = 0uL;
		host.SceneReadyEpochByConnectionId.Clear();
		host.Phase = HostPvpAgreementPhase.WaitingForScenes;
		host.DeadlineTimestamp = DeadlineAfter(90.0);
		host.NativeLifecycleDeadlineTimestamp = 0L;
		try
		{
			BroadcastHostPvpSceneReadyRequest(host, isRetry: false);
			log.LogInfo((object)("PVP scene-generation readiness requested: epoch=" + host.SceneGenerationEpoch + ", reason=" + reason + "."));
			return true;
		}
		catch (Exception ex)
		{
			error = "host scene-generation request failed: " + ex.GetType().Name + ": " + ex.Message;
			return false;
		}
	}

	private void AcceptRemotePvpSceneReadyRequest(RemotePvpAgreement remote, ulong sceneGenerationEpoch)
	{
		if (remote == null || remote.Identity == null || remote != remotePvpAgreement || !remote.ContentCommitted)
		{
			throw new InvalidOperationException("host scene-generation request had no committed remote session");
		}
		if (sceneGenerationEpoch < remote.RequestedSceneGenerationEpoch)
		{
			log.LogWarning((object)("Remote PVP peer ignored stale scene-generation readiness request: receivedEpoch=" + sceneGenerationEpoch + ", currentEpoch=" + remote.RequestedSceneGenerationEpoch + "."));
			return;
		}
		if (sceneGenerationEpoch == remote.RequestedSceneGenerationEpoch)
		{
		...
		{
			log.LogWarning((object)("Remote PVP peer ignored already-acknowledged stale scene-generation readiness request: receivedEpoch=" + sceneGenerationEpoch + ", acknowledgedEpoch=" + remote.SceneReadySentEpoch + "."));
			return;
		}
		if (remote.RequestedSceneGenerationEpoch == ulong.MaxValue || sceneGenerationEpoch != remote.RequestedSceneGenerationEpoch + 1)
		{
			FailRemotePvpAgreement("host scene-generation request was not the next monotonic epoch", sendReject: true);
			return;
		}
		if (remote.RequestedSceneGenerationEpoch != remote.SceneReadySentEpoch)
		{
			FailRemotePvpAgreement("host advanced the scene-generation epoch before the prior acknowledgement completed", sendReject: true);
			return;
		}
		if (remote.SceneReadySentLocalGeneration == ulong.MaxValue)
		{
			FailRemotePvpAgreement("remote scene-generation request mapping overflowed", sendReject: true);
			return;
		}
		remote.RequestedSceneGenerationEpoch = sceneGenerationEpoch;
		remote.RequestedLocalSceneGeneration = remote.SceneReadySentLocalGeneration + 1;
		remote.SceneReadySent = false;
		remote.NativeOwnerAdopted = false;
		remote.NativeReadinessInitialized = false;
		remote.DeadlineTimestamp = DeadlineAfter(90.0);
		log.LogInfo((object)("Remote PVP peer accepted scene-generation readiness request: epoch=" + sceneGenerationEpoch + ", localGeneration=" + remote.LocalSceneGeneration + ", requiredLocalGeneration=" + remote.RequestedLocalSceneGeneration + "."));
		TrySendRemotePvpSceneReadyAcknowledgement(remote, allowResend: false);
	}

	private void TrySendRemotePvpSceneReadyAcknowledgement(RemotePvpAgreement remote, bool allowResend)
	{
		if (remote == null || remote != remotePvpAgreement || !remote.ContentCommitted || !remote.LocalSceneReady || remote.LocalReadySceneGeneration != remote.LocalSceneGeneration || remote.RequestedSceneGenerationEpoch == 0L)
		{
			return;
		}
		if (remote.LocalSceneGeneration > remote.RequestedLocalSceneGeneration)
		{
			if (remote.RequestedSceneGenerationEpoch != remote.SceneReadySentEpoch)
			{
				FailRemotePvpAgreement("remote exact package scene generation advanced beyond the host scene-generation request", sendReject: true);
			}
		}
		else
		{
			...
			bool flag = remote.SceneReadySent && remote.SceneReadySentEpoch == remote.RequestedSceneGenerationEpoch && remote.SceneReadySentLocalGeneration == remote.LocalSceneGeneration;
			if ((!flag || allowResend) && (flag || remote.RequestedSceneGenerationEpoch > remote.SceneReadySentEpoch))
			{
				SendRemotePvpControl(PvpAgreementMessageKind.SceneReady, string.Empty, null, remote.RequestedSceneGenerationEpoch);
				if (flag)
				{
					log.LogInfo((object)("Remote PVP peer resent the exact scene-ready acknowledgement on host request: sceneGenerationEpoch=" + remote.RequestedSceneGenerationEpoch + ", localGeneration=" + remote.LocalSceneGeneration + "."));
				}
				else
				{
					...
					remote.DeadlineTimestamp = DeadlineAfter(90.0);
					log.LogInfo((object)"Remote PVP peer constructed and registered the exact native PvpGameode template; scene-ready acknowledgement sent.");
					log.LogInfo((object)("Remote PVP scene-ready acknowledgement identity: sceneGenerationEpoch=" + remote.SceneReadySentEpoch + ", localGeneration=" + remote.SceneReadySentLocalGeneration + "."));
				}
			}
		}
	}

	private static bool IsHostLocalSceneReadyForCurrentEpoch(HostPvpAgreement host)
	{
		if (host != null && host.SceneGenerationEpoch != 0L && host.LocalSceneReady)
		{
			return host.LocalSceneReadyEpoch == host.SceneGenerationEpoch;
		}
		return false;
	}

	private static int CountRequiredPvpPeersSceneReadyForCurrentEpoch(HostPvpAgreement host)
	{
		if (host == null || host.SceneGenerationEpoch == 0L)
		{
			return 0;
		}
		int num = 0;
		foreach (int requiredConnectionId in host.RequiredConnectionIds)
		{
			if (host.SceneReadyEpochByConnectionId.TryGetValue(requiredConnectionId, out var value) && value == host.SceneGenerationEpoch)
			{
				num++;
			}
		}
	...
	private static bool AreRequiredPvpPeersSceneReadyForCurrentEpoch(HostPvpAgreement host)
	{
		if (host != null && host.RequiredConnectionIds.Count != 0)
		{
			return CountRequiredPvpPeersSceneReadyForCurrentEpoch(host) == host.RequiredConnectionIds.Count;
		}
		return false;
	}

	private static bool IsRemoteSceneReadyForCurrentRequest(RemotePvpAgreement remote)
	{
		if (remote != null && remote.RequestedSceneGenerationEpoch != 0L && remote.SceneReadySent && remote.SceneReadySentEpoch == remote.RequestedSceneGenerationEpoch && remote.SceneReadySentLocalGeneration == remote.RequestedLocalSceneGeneration)
		{
			return remote.LocalSceneGeneration == remote.RequestedLocalSceneGeneration;
		}
		return false;
	}
	...
	private void NotifyPvpSceneLoading(ActiveMapOperation operation)
	{
		...
		ModdedOperationDefinition operation2 = operation.Operation;
		if ((int)((operation2 != null) ? new ModdedOperationMode?(operation2.Mode) : ((ModdedOperationMode?)null)).GetValueOrDefault() != 1)
		{
			return;
		}
		RemotePvpAgreement remotePvpAgreement = this.remotePvpAgreement;
		...
		{
			FailRemotePvpAgreement("remote exact package scene load had no scene handle", sendReject: true);
			operation.NetworkSpawnFailed = true;
			return;
		}
		if (remotePvpAgreement.AwaitingLocalSceneGeneration || remotePvpAgreement.LocalSceneGeneration == 0L || remotePvpAgreement.LastObservedSceneHandle != operation.SceneHandle)
		{
			if (remotePvpAgreement.LocalSceneGeneration == ulong.MaxValue)
			{
				FailRemotePvpAgreement("remote local scene-generation counter overflowed", sendReject: true);
				operation.NetworkSpawnFailed = true;
				return;
			}
			remotePvpAgreement.LocalSceneGeneration++;
			remotePvpAgreement.LastObservedSceneHandle = operation.SceneHandle;
			remotePvpAgreement.AwaitingLocalSceneGeneration = false;
			remotePvpAgreement.LocalSceneReady = false;
			remotePvpAgreement.LocalReadySceneGeneration = 0uL;
			remotePvpAgreement.SceneReadySent = false;
			remotePvpAgreement.NativeOwnerAdopted = false;
			remotePvpAgreement.NativeReadinessInitialized = false;
			log.LogInfo((object)("Remote PVP peer observed exact package scene generation=" + remotePvpAgreement.LocalSceneGeneration + ", sceneHandle=" + operation.SceneHandle + "."));
		}
		remotePvpAgreement.DeadlineTimestamp = DeadlineAfter(90.0);
	}

	private void NotifyPvpScenePreparationFailed(ActiveMapOperation operation, string reason)
	{
		//IL_001b: Unknown result type (might be due to invalid IL or missing references)
		//IL_0028: Unknown result type (might be due to invalid IL or missing references)
		//IL_002e: Invalid comparison between Unknown and I4
		...
		if ((int)((operation2 != null) ? new ModdedOperationMode?(operation2.Mode) : ((ModdedOperationMode?)null)).GetValueOrDefault() == 1)
		{
			...
			{
				FailHostPvpAgreement("host PVP scene preparation failed: " + reason, sendCancel: true);
			}
			else if (remotePvpAgreement != null)
			{
				FailRemotePvpAgreement("remote PVP scene preparation failed: " + reason, sendReject: true);
			}
			operation.NetworkSpawnFailed = true;
		}
	}
	...
	private void ResetPvpSceneAgreementForReload(ActiveMapOperation operation)
	{
		...
		HostPvpAgreement hostPvpAgreement = this.hostPvpAgreement;
		if (NetworkServer.active && hostPvpAgreement != null && OperationMatchesPvpIdentity(operation, hostPvpAgreement.Identity) && !TryBeginHostPvpSceneGeneration(hostPvpAgreement, "exact package scene restart", out var error))
		{
			FailHostPvpAgreement(error, sendCancel: true);
			operation.NetworkSpawnFailed = true;
			return;
		...
		RemotePvpAgreement remotePvpAgreement = this.remotePvpAgreement;
		if (!NetworkServer.active && remotePvpAgreement != null && OperationMatchesPvpIdentity(operation, remotePvpAgreement.Identity))
		{
			remotePvpAgreement.SceneReadySent = false;
			remotePvpAgreement.LocalSceneReady = false;
			remotePvpAgreement.LocalReadySceneGeneration = 0uL;
			remotePvpAgreement.AwaitingLocalSceneGeneration = true;
			remotePvpAgreement.NativeOwnerAdopted = false;
			remotePvpAgreement.NativeReadinessInitialized = false;
			remotePvpAgreement.DeadlineTimestamp = DeadlineAfter(90.0);
		}
	...
	private bool CompletePvpPeerAgreementOnNativeReturn(ActiveMapOperation operation)
	{
		//IL_001b: Unknown result type (might be due to invalid IL or missing references)
		//IL_0028: Unknown result type (might be due to invalid IL or missing references)
		//IL_002e: Invalid comparison between Unknown and I4
		if (operation != null)
		{
			...
			if ((int)((operation2 != null) ? new ModdedOperationMode?(operation2.Mode) : ((ModdedOperationMode?)null)).GetValueOrDefault() == 1)
			{
				...
				if (!string.Equals(networkSceneName, "Operation Room", StringComparison.OrdinalIgnoreCase) && !(networkSceneName ?? string.Empty).EndsWith("/Operation Room.unity", StringComparison.OrdinalIgnoreCase) && !(networkSceneName ?? string.Empty).EndsWith("\\Operation Room.unity", StringComparison.OrdinalIgnoreCase))
				{
					return false;
				}
				bool flag = false;
				if (hostPvpAgreement != null && OperationMatchesPvpIdentity(operation, hostPvpAgreement.Identity))
				{
					hostPvpAgreement = null;
					flag = true;
				}
				if (remotePvpAgreement != null && OperationMatchesPvpIdentity(operation, remotePvpAgreement.Identity))
				{
					remotePvpAgreement = null;
					flag = true;
				}
				...
				if (flag)
				{
					log.LogInfo((object)"PVP peer agreement closed after the shipped native return to Operation Room; restart state was not armed.");
				}
				else
				{
					log.LogInfo((object)"PVP native return reached Operation Room after its peer agreement had already been closed; restart state was not armed.");
				}
				return true;
			}
		}
	...
	private bool IsPvpNetworkSpawnAuthorized(ActiveMapOperation operation)
	{
		...
		if (operation != null)
		{
			...
			if ((int)((operation2 != null) ? new ModdedOperationMode?(operation2.Mode) : ((ModdedOperationMode?)null)).GetValueOrDefault() == 1)
			{
				...
				if (hostPvpAgreement.Phase != HostPvpAgreementPhase.ReadyToSpawn && hostPvpAgreement.Phase != HostPvpAgreementPhase.SpawnAuthorized)
				{
					return false;
				}
				string error = string.Empty;
				if (!IsHostLocalSceneReadyForCurrentEpoch(hostPvpAgreement) || !AreRequiredPvpPeersSceneReadyForCurrentEpoch(hostPvpAgreement) || !TryValidateHostPvpMembership(hostPvpAgreement, out error))
				{
					FailHostPvpAgreement(string.IsNullOrEmpty(error) ? "PVP scene-ready barrier changed before native spawn" : error, sendCancel: true);
					operation.NetworkSpawnFailed = true;
					return false;
				}
				if (DeadlineExpired(hostPvpAgreement.DeadlineTimestamp))
				{
					FailHostPvpAgreement("timed out authorizing the exact native PVP owner spawn", sendCancel: true);
					operation.NetworkSpawnFailed = true;
					return false;
				}
				if (!TryValidateCurrentPvpTeamCapacity(operation, hostPvpAgreement.RequiredConnectionIds.Count + 1, out var error2))
				{
					FailHostPvpAgreement(error2, sendCancel: true);
					operation.NetworkSpawnFailed = true;
					return false;
				}
				hostPvpAgreement.Phase = HostPvpAgreementPhase.SpawnAuthorized;
				return true;
			}
		}
		return true;
	...
	private static bool TryValidateCurrentPvpTeamCapacity(ActiveMapOperation operation, int agreedPlayerCount, out string error)
	{
		error = string.Empty;
		StandalonePvpGameMode standalonePvpGameMode = operation?.GameModeComponent as StandalonePvpGameMode;
		GameManagerNetwork val = GameManagerNetwork.instance;
		if ((Object)(object)standalonePvpGameMode == (Object)null || ((PvpGameode)standalonePvpGameMode).Team1SpawnPoints == null || ((PvpGameode)standalonePvpGameMode).Team2SpawnPoints == null || (Object)(object)val == (Object)null || val.playerMasters == null)
		{
			error = "native PVP team/spawn registries are unavailable before spawn";
			return false;
		}
		if (val.playerMasters.Count != agreedPlayerCount)
		{
			error = "native PVP player registry count " + val.playerMasters.Count + " does not match agreed lobby count " + agreedPlayerCount;
			return false;
		}
		int num = 0;
		int num2 = 0;
		for (int i = 0; i < val.playerMasters.Count; i++)
		{
			PlayerMaster obj = val.playerMasters[i];
			int? obj2;
			if (obj == null)
			{
				obj2 = null;
			...
			{
				TeamIdentifier myTeamIdentifier = obj.MyTeamIdentifier;
				obj2 = ((myTeamIdentifier != null) ? new int?(myTeamIdentifier.TeamID) : ((int?)null));
			}
			int? num3 = obj2;
			...
			case 2:
				...
				break;
			default:
				error = "native PVP player at index " + i + " has unresolved team ID " + valueOrDefault;
				return false;
			}
		}
		if (num > ((PvpGameode)standalonePvpGameMode).Team1SpawnPoints.Count || num2 > ((PvpGameode)standalonePvpGameMode).Team2SpawnPoints.Count)
		{
			error = "native PVP team population exceeds authored spawn capacity: Team1=" + num + "/" + ((PvpGameode)standalonePvpGameMode).Team1SpawnPoints.Count + ", Team2=" + num2 + "/" + ((PvpGameode)standalonePvpGameMode).Team2SpawnPoints.Count;
			return false;
		}
		return true;
	}
		...
		if (operation != null)
		{
			...
			if ((int)((operation2 != null) ? new ModdedOperationMode?(operation2.Mode) : ((ModdedOperationMode?)null)).GetValueOrDefault() == 1)
			{
				if (NetworkServer.active)
				{
					...
					if (hostPvpAgreement != null && OperationMatchesPvpIdentity(operation, hostPvpAgreement.Identity))
					{
						if (hostPvpAgreement.Phase != HostPvpAgreementPhase.ReadyToSpawn)
						{
							return hostPvpAgreement.Phase == HostPvpAgreementPhase.SpawnAuthorized;
						}
						return true;
					}
					return false;
				...
				RemotePvpAgreement remotePvpAgreement = this.remotePvpAgreement;
				if (remotePvpAgreement != null && remotePvpAgreement.ContentCommitted && IsRemoteSceneReadyForCurrentRequest(remotePvpAgreement))
				{
					return OperationMatchesPvpIdentity(operation, remotePvpAgreement.Identity);
				}
				return false;
			}
		}
	...
	private static bool OperationMatchesPvpIdentity(ActiveMapOperation operation, PvpPeerIdentity identity)
	{
		if (operation != null && identity != null && operation.Map != null && operation.Operation != null && operation.SceneSelection != null && string.Equals(operation.Map.Id, identity.MapId, StringComparison.Ordinal) && string.Equals(operation.Map.PackageId, identity.PackageId, StringComparison.Ordinal) && string.Equals(operation.Map.PackageVersion, identity.PackageVersion, StringComparison.Ordinal) && string.Equals(operation.Map.PackageContentId, identity.PackageContentId, StringComparison.Ordinal) && string.Equals(operation.Operation.Id, identity.OperationId, StringComparison.Ordinal) && string.Equals(operation.Operation.SpawnSetId, identity.SpawnSetId, StringComparison.Ordinal) && string.Equals(operation.TimeCode, identity.TimeCode, StringComparison.Ordinal) && string.Equals(operation.SceneSelection.Id, identity.VariantId, StringComparison.Ordinal))
		{
			return string.Equals(operation.SceneSelection.ScenePath, identity.ScenePath, StringComparison.Ordinal);
		}
		return false;
	}

	private Dictionary<int, NetworkConnectionToClient> CaptureAuthenticatedRemotePvpConnections(bool requireReady)
	{
		if (!NetworkServer.active || NetworkServer.connections == null)
		{
			throw new InvalidOperationException("Mirror server connection registry is unavailable");
		}
		Dictionary<int, NetworkConnectionToClient> dictionary = new Dictionary<int, NetworkConnectionToClient>();
		int num = ((NetworkServer.localConnection == null) ? NetworkConnection.LocalConnectionId : ((NetworkConnection)NetworkServer.localConnection).connectionId);
		...
		try
		{
			while (enumerator.MoveNext())
			{
				...
				if (value != null && current.Key != num && !(value is LocalConnectionToClient))
				{
					if (!((NetworkConnection)value).isAuthenticated || (requireReady && !((NetworkConnection)value).isReady))
					{
						throw new InvalidOperationException("remote peer " + current.Key + " is not authenticated and ready");
					}
	...
	private bool TryValidateHostPvpMembership(HostPvpAgreement host, out string error)
	{
		error = string.Empty;
		try
		{
			Dictionary<int, NetworkConnectionToClient> dictionary = CaptureAuthenticatedRemotePvpConnections(requireReady: false);
			if (!host.RequiredConnectionIds.SetEquals(dictionary.Keys))
			{
			...
			foreach (int requiredConnectionId in host.RequiredConnectionIds)
			{
				if (!host.RequiredConnections.TryGetValue(requiredConnectionId, out var value) || !dictionary.TryGetValue(requiredConnectionId, out var value2) || !SamePvpNetworkConnection((NetworkConnection)(object)value, (NetworkConnection)(object)value2))
				{
					error = "PVP lobby connection identity changed during peer agreement for connection=" + requiredConnectionId;
					return false;
				}
			}
			return true;
	...
	private static bool SamePvpNetworkConnection(NetworkConnection left, NetworkConnection right)
	{
		if (left == right)
		{
			return true;
	...
	private void FailHostPvpAgreement(string reason, bool sendCancel)
	{
		...
		if (activeOperation != null && OperationMatchesPvpIdentity(activeOperation, hostPvpAgreement.Identity))
		{
			activeOperation.NetworkSpawnFailed = true;
			if (hostPvpAgreement.NativeLaunchInvoked || activeOperation.NativeLaunchInvoked)
			{
				RequestNativePvpAbortReturn(activeOperation);
			}
		}
		SetNativeConfirmationLoadingState(hostPvpAgreement.Presentation, loading: false);
		this.hostPvpAgreement = null;
		log.LogError((object)("PVP peer agreement failed closed: " + reason + "."));
	}
	...
	private void FailRemotePvpAgreement(string reason, bool sendReject)
	{
		...
		ClearRemotePvpAgreement(reason);
		log.LogError((object)("Remote PVP peer agreement failed closed: " + reason + "."));
	}

	private void ClearRemotePvpAgreement(string reason)
	{
		RemotePvpAgreement remotePvpAgreement = this.remotePvpAgreement;
		if (remotePvpAgreement != null && activeOperation != null && OperationMatchesPvpIdentity(activeOperation, remotePvpAgreement.Identity))
		{
			if (!activeOperation.NativeLaunchInvoked && activeOperation.SceneHandle == 0 && !remotePvpAgreement.ContentCommitted)
			{
				activeOperation = null;
			}
			else
	...
	private void RequestNativePvpAbortReturn(ActiveMapOperation operation)
	{
		...
		if (NetworkServer.active && (Object)(object)GameManagerNetwork.instance != (Object)null)
		{
			try
			{
				GameManagerNetwork.instance.EndOperation();
				log.LogWarning((object)"PVP agreement abort entered the shipped GameManagerNetwork.EndOperation return to Operation Room.");
				return;
			}
			catch (Exception ex)
			{
				log.LogError((object)("PVP agreement abort could not enter the shipped Operation Room return: " + ex.GetType().Name + ": " + ex.Message + "."));
			}
		}
		if (!NetworkServer.active && NetworkClient.active)
		{
			try
			{
				...
				log.LogWarning((object)"PVP agreement abort disconnected the remote peer before exact local scene teardown.");
			}
			catch (Exception ex2)
			{
				log.LogError((object)("PVP agreement abort could not disconnect the remote peer: " + ex2.GetType().Name + ": " + ex2.Message + "."));
			}
		}
		...
		try
		{
			...
			log.LogWarning((object)("PVP agreement abort requested exact local scene unload because the native network return was unavailable: scene=" + ((Scene)(ref val)).name + "."));
		}
		catch (Exception ex3)
		{
			log.LogError((object)("PVP agreement abort exact-scene unload failed: " + ex3.GetType().Name + ": " + ex3.Message + "."));
		}
	}

	private void FailActivePvpNativeLifecycle(ActiveMapOperation operation, string reason)
	{
		//IL_001b: Unknown result type (might be due to invalid IL or missing references)
		//IL_0028: Unknown result type (might be due to invalid IL or missing references)
		//IL_002e: Invalid comparison between Unknown and I4
		...
		if ((int)((operation2 != null) ? new ModdedOperationMode?(operation2.Mode) : ((ModdedOperationMode?)null)).GetValueOrDefault() == 1)
		{
			...
			if (NetworkServer.active && hostPvpAgreement != null && OperationMatchesPvpIdentity(operation, hostPvpAgreement.Identity))
			{
				FailHostPvpAgreement("host native PVP lifecycle failed: " + reason, sendCancel: true);
			}
			else if (!NetworkServer.active && remotePvpAgreement != null && OperationMatchesPvpIdentity(operation, remotePvpAgreement.Identity))
			{
				FailRemotePvpAgreement("remote native PVP lifecycle failed: " + reason, sendReject: true);
			}
			else
			{
	...
	private void NotifyPvpNativeLaunchInvoked(ActiveMapOperation operation)
	{
		...
		{
			ModdedOperationDefinition operation2 = operation.Operation;
			if ((int)((operation2 != null) ? new ModdedOperationMode?(operation2.Mode) : ((ModdedOperationMode?)null)).GetValueOrDefault() == 1 && NetworkServer.active && hostPvpAgreement != null && OperationMatchesPvpIdentity(operation, hostPvpAgreement.Identity))
			{
	...
	private void NotifyPvpNativeOwnerAdopted(ActiveMapOperation operation)
	{
		...
		{
			RemotePvpAgreement remotePvpAgreement = this.remotePvpAgreement;
			if (!NetworkServer.active && remotePvpAgreement != null && IsRemoteSceneReadyForCurrentRequest(remotePvpAgreement) && OperationMatchesPvpIdentity(operation, remotePvpAgreement.Identity))
			{
				remotePvpAgreement.NativeOwnerAdopted = true;
				remotePvpAgreement.DeadlineTimestamp = DeadlineAfter(90.0);
				log.LogInfo((object)"Remote PVP peer adopted the agreed native owner; awaiting the shipped readiness lifecycle.");
			}
		}
	}
	...
	private void NotifyPvpNativeReadinessInitialized(ActiveMapOperation operation)
	{
		...
		if ((int)((operation2 != null) ? new ModdedOperationMode?(operation2.Mode) : ((ModdedOperationMode?)null)).GetValueOrDefault() == 1)
		{
			...
			{
				remotePvpAgreement.NativeOwnerAdopted = true;
				remotePvpAgreement.NativeReadinessInitialized = true;
				remotePvpAgreement.DeadlineTimestamp = 0L;
				log.LogInfo((object)"Remote PVP peer completed the agreed native readiness lifecycle.");
			}
		}
	}
	...
	private void NotifyPvpNetworkOwnerSpawned(ActiveMapOperation operation)
	{
		...
		if (operation != null)
		{
			...
			if ((int)((operation2 != null) ? new ModdedOperationMode?(operation2.Mode) : ((ModdedOperationMode?)null)).GetValueOrDefault() == 1 && NetworkServer.active && hostPvpAgreement != null && OperationMatchesPvpIdentity(operation, hostPvpAgreement.Identity))
			{
				hostPvpAgreement.NativeLifecycleDeadlineTimestamp = (operation.AllPlayersLoaded ? 0 : DeadlineAfter(90.0));
				log.LogInfo((object)("PVP native owner spawned; bounded native readiness lifecycle " + (operation.AllPlayersLoaded ? "was already complete" : "started") + " for operation=" + operation.Operation.Id + "."));
			}
		}
	}
	...
	private void NotifyPvpAllPlayersLoaded(ActiveMapOperation operation)
	{
		...
		if (operation != null)
		{
			...
			if ((int)((operation2 != null) ? new ModdedOperationMode?(operation2.Mode) : ((ModdedOperationMode?)null)).GetValueOrDefault() == 1 && NetworkServer.active && hostPvpAgreement != null && OperationMatchesPvpIdentity(operation, hostPvpAgreement.Identity))
			{
				hostPvpAgreement.NativeLifecycleDeadlineTimestamp = 0L;
				log.LogInfo((object)"PVP shipped all-players-loaded lifecycle passed before the native-start deadline.");
			}
		}
	}

	private void SendPvpOffer(NetworkConnection connection, PvpPeerIdentity identity)
	{
		SendPvpEnvelope(connection, delegate(NetworkWriter writer)
		{
			NetworkWriterExtensions.WriteUShort(writer, (ushort)2);
			NetworkWriterExtensions.WriteByte(writer, (byte)1);
			WritePvpIdentity(writer, identity);
		});
	}

	private void BroadcastHostPvpControl(PvpAgreementMessageKind kind, string reason)
	{
		HostPvpAgreement hostPvpAgreement = this.hostPvpAgreement;
		if (hostPvpAgreement == null || !NetworkServer.active)
		{
			return;
		}
		Dictionary<int, NetworkConnectionToClient> dictionary = CaptureAuthenticatedRemotePvpConnections(requireReady: false);
		foreach (int requiredConnectionId in hostPvpAgreement.RequiredConnectionIds)
		{
			if (dictionary.TryGetValue(requiredConnectionId, out var value) && hostPvpAgreement.RequiredConnections.TryGetValue(requiredConnectionId, out var value2) && SamePvpNetworkConnection((NetworkConnection)(object)value2, (NetworkConnection)(object)value))
			{
				SendPvpControl((NetworkConnection)(object)value, kind, hostPvpAgreement.Identity, reason, 0uL);
			}
		}
	}

	private void BroadcastHostPvpSceneReadyRequest(HostPvpAgreement host, bool isRetry)
	{
		if (host == null || host != hostPvpAgreement || !NetworkServer.active || host.SceneGenerationEpoch == 0L)
		{
			throw new InvalidOperationException("host scene-generation request lost agreement ownership");
		}
		Dictionary<int, NetworkConnectionToClient> dictionary = CaptureAuthenticatedRemotePvpConnections(requireReady: false);
		foreach (int requiredConnectionId in host.RequiredConnectionIds)
		{
			if (!dictionary.TryGetValue(requiredConnectionId, out var value) || !host.RequiredConnections.TryGetValue(requiredConnectionId, out var value2) || !SamePvpNetworkConnection((NetworkConnection)(object)value2, (NetworkConnection)(object)value))
			{
				throw new InvalidOperationException("required PVP peer " + requiredConnectionId + " was unavailable for scene-generation request");
			}
			SendHostPvpSceneReadyRequest((NetworkConnection)(object)value, host, isRetry);
		}
		host.SceneReadyRequestRetryTimestamp = DeadlineAfter(5.0);
		if (isRetry)
		{
			log.LogInfo((object)("PVP scene-generation readiness request resent: epoch=" + host.SceneGenerationEpoch + ", ready=" + CountRequiredPvpPeersSceneReadyForCurrentEpoch(host) + "/" + host.RequiredConnectionIds.Count + "."));
		}
	}

	private void SendHostPvpSceneReadyRequest(NetworkConnection connection, HostPvpAgreement host, bool isRetry)
	{
		if (connection == null || host == null || host.Identity == null || host.SceneGenerationEpoch == 0L)
		{
			throw new InvalidOperationException("host scene-generation request was incomplete");
		}
		SendPvpControl(connection, PvpAgreementMessageKind.SceneReadyRequest, host.Identity, string.Empty, host.SceneGenerationEpoch);
	}

	private void SendRemotePvpControl(PvpAgreementMessageKind kind, string reason, PvpPeerIdentity identityOverride = null, ulong sceneGenerationEpoch = 0uL)
	{
		PvpPeerIdentity pvpPeerIdentity = identityOverride ?? remotePvpAgreement?.Identity;
		if (pvpPeerIdentity != null && NetworkClient.active && NetworkClient.connection != null)
		{
			SendPvpControl(NetworkClient.connection, kind, pvpPeerIdentity, reason, sceneGenerationEpoch);
		}
	}

	private void SendPvpControl(NetworkConnection connection, PvpAgreementMessageKind kind, PvpPeerIdentity identity, string reason, ulong sceneGenerationEpoch = 0uL)
	{
		SendPvpEnvelope(connection, delegate(NetworkWriter writer)
		{
			NetworkWriterExtensions.WriteUShort(writer, (ushort)2);
			...
			NetworkWriterExtensions.WriteString(writer, BoundPvpString(reason, 512));
		});
	}

	private static void SendPvpEnvelope(NetworkConnection connection, Action<NetworkWriter> writePayload)
	{
		...
		try
		{
			NetworkWriterExtensions.WriteUShort((NetworkWriter)(object)val, (ushort)19791);
			writePayload((NetworkWriter)(object)val);
			if (((NetworkWriter)val).Position > 8192)
			{
				throw new InvalidOperationException("PVP agreement envelope exceeded its byte limit");
			}
			connection.Send(((NetworkWriter)val).ToArraySegment(), Channels.Reliable);
		}
		finally
		{
	...
	private static PvpPeerIdentity CreatePvpPeerIdentity(ModdedMapDefinition map, ModdedOperationDefinition operation, string timeCode, SceneVariantSelection selection, string nonce)
	{
		//IL_0044: Unknown result type (might be due to invalid IL or missing references)
		//IL_004a: Invalid comparison between Unknown and I4
		//IL_011e: Unknown result type (might be due to invalid IL or missing references)
		//IL_0128: Expected I4, but got Unknown
		if (!TryResolveLocalRuntimeIdentity(map, out var identity, out var error))
		{
			throw new InvalidOperationException("exact runtime binary identity failed closed: " + error);
		}
		if (string.IsNullOrWhiteSpace(nonce) || nonce.Length > 64 || map == null || operation == null || selection == null || !SelectionBelongsToMap(map, selection) || (int)operation.Mode != 1 || !string.Equals(operation.MapId, map.Id, StringComparison.Ordinal) || !operation.SupportedTimeCodes.Contains<string>(timeCode, StringComparer.Ordinal))
		{
			throw new InvalidOperationException("local PVP offer identity was incomplete or not owned by the map");
		}
		PvpPeerIdentity obj = new PvpPeerIdentity
		{
			Nonce = nonce,
			FrameworkVersion = "0.3.29",
			FrameworkSha256 = identity.FrameworkSha256,
			...
			Capabilities = "exact-content-v2;runtime-binary-sha256-v1;package-runtime-ready-v1;remote-preload-v1;scene-ready-epoch-v1;native-pvp-v1",
			...
			CompanionPluginGuid = identity.CompanionPluginGuid,
			CompanionPluginVersion = identity.CompanionPluginVersion,
			CompanionSha256 = identity.CompanionSha256,
			CompanionReadyMarkerName = identity.CompanionReadyMarkerName,
			CompanionFailureMarkerName = identity.CompanionFailureMarkerName
		};
		obj.Digest = ComputePvpIdentityDigest(obj);
		return obj;
	}

	private static void WritePvpIdentity(NetworkWriter writer, PvpPeerIdentity identity)
	{
	...
	private static PvpPeerIdentity ReadPvpIdentity(NetworkReader reader)
	{
		PvpPeerIdentity obj = new PvpPeerIdentity
		{
			...
			ScenePath = ReadBoundedPvpString(reader, 1024, "scenePath"),
			...
			CompanionPluginGuid = ReadBoundedPvpString(reader, 128, "companionPluginGuid"),
			CompanionPluginVersion = ReadBoundedPvpString(reader, 64, "companionPluginVersion"),
			CompanionSha256 = ReadBoundedPvpString(reader, 64, "companionSha256"),
			CompanionReadyMarkerName = ReadBoundedPvpString(reader, 128, "companionReadyMarkerName"),
			CompanionFailureMarkerName = ReadBoundedPvpString(reader, 128, "companionFailureMarkerName"),
			Digest = ReadBoundedPvpString(reader, 128, "digest")
		};
		ValidatePvpCompanionIdentity(obj);
		return obj;
	}
	...
	private static string ComputePvpIdentityDigest(PvpPeerIdentity identity)
	{
		...
		{
			((ushort)2).ToString(CultureInfo.InvariantCulture),
	...
	private static bool TryResolveLocalRuntimeIdentity(ModdedMapDefinition map, out RuntimeBinaryIdentity identity, out string error)
	{
		identity = null;
		error = string.Empty;
		try
		{
			if (!TryHashLoadedAssembly(typeof(CerberusNativeTabFix).Assembly, "OperatorModdedOperations", null, out var sha, out error))
			{
				return false;
			}
			if (!TryHashLoadedAssembly(typeof(OperatorApi).Assembly, "OperatorModAPI", null, out var sha2, out error))
			{
				return false;
			}
			if (!TryResolveLoadedPlugin("operator.modapi", OperatorApi.ApiVersion, "OperatorModAPI.BepInEx.OperatorApiPlugin", "OperatorModAPI.BepInEx", null, out var sha3, out error))
			{
				return false;
			}
			RuntimeBinaryIdentity runtimeBinaryIdentity = new RuntimeBinaryIdentity
			{
				FrameworkSha256 = sha,
				ApiCoreSha256 = sha2,
				ApiHostSha256 = sha3,
				CompanionPluginGuid = "none",
				CompanionPluginVersion = "none",
				CompanionSha256 = "none",
				CompanionReadyMarkerName = "none",
				CompanionFailureMarkerName = "none"
			};
			ModdedRuntimeCompanionDefinition val = ((map != null) ? map.RuntimeCompanion : null);
			if (val != null)
			{
				if (!TryResolveLoadedPlugin(val.PluginGuid, val.PluginVersion, null, null, val.Sha256, out var sha4, out error))
				{
					return false;
				}
				runtimeBinaryIdentity.CompanionPluginGuid = val.PluginGuid;
				runtimeBinaryIdentity.CompanionPluginVersion = val.PluginVersion;
				runtimeBinaryIdentity.CompanionSha256 = sha4;
				runtimeBinaryIdentity.CompanionReadyMarkerName = val.ReadyMarkerName;
				runtimeBinaryIdentity.CompanionFailureMarkerName = val.FailureMarkerName;
			}
			identity = runtimeBinaryIdentity;
			return true;
		}
		catch (Exception ex) when (((ex is IOException || ex is UnauthorizedAccessException || ex is ArgumentException || ex is InvalidOperationException || ex is CryptographicException || ex is BadImageFormatException) ? 1 : 0) != 0)
		{
			error = ex.GetType().Name + ": " + ex.Message;
			return false;
		}
	...
	private static bool TryResolveLoadedPlugin(string pluginGuid, string expectedVersion, string expectedTypeName, string expectedAssemblyName, string expectedSha256, out string sha256, out string error)
	{
		sha256 = string.Empty;
		error = string.Empty;
		IL2CPPChainloader val = IL2CPPChainloader.Instance;
		if (((BaseChainloader<BasePlugin>)(object)val)?.Plugins == null || !((BaseChainloader<BasePlugin>)(object)val).Plugins.TryGetValue(pluginGuid, out var value) || value == null || value.Metadata == null || value.Instance == null)
		{
			error = "required loaded plugin '" + pluginGuid + "' is unavailable";
			return false;
		}
		string a = ((object)value.Metadata).GetType().GetProperty("Version", BindingFlags.Instance | BindingFlags.Public)?.GetValue(value.Metadata)?.ToString();
		if (!string.Equals(value.Metadata.GUID, pluginGuid, StringComparison.Ordinal) || !string.Equals(a, expectedVersion, StringComparison.Ordinal))
		{
			error = "loaded plugin GUID/version mismatch for '" + pluginGuid + "'";
			return false;
		}
		Type type = value.Instance.GetType();
		if (!string.Equals(value.TypeName, type.FullName, StringComparison.Ordinal))
		{
			error = "loaded plugin registry/type identity mismatch for '" + pluginGuid + "'";
			return false;
		}
		if (expectedTypeName != null && !string.Equals(type.FullName, expectedTypeName, StringComparison.Ordinal))
		{
			error = "loaded plugin type mismatch for '" + pluginGuid + "'";
			return false;
		}
		if (TryHashLoadedAssembly(type.Assembly, expectedAssemblyName, value.Location, out sha256, out error))
		{
			if (expectedSha256 != null)
			{
				return RequireExpectedPvpSha256(pluginGuid, sha256, expectedSha256, out error);
			}
			return true;
		}
		return false;
	...
	private static bool RequireExpectedPvpSha256(string pluginGuid, string actual, string expected, out string error)
	{
		if (IsLowercasePvpSha256(expected) && string.Equals(actual, expected, StringComparison.Ordinal))
		{
			error = string.Empty;
			return true;
		}
	...
	private static bool TryHashLoadedAssembly(Assembly assembly, string expectedAssemblyName, string authoritativePath, out string sha256, out string error)
	{
		sha256 = string.Empty;
		error = string.Empty;
		if (assembly == null)
		...
		if (expectedAssemblyName != null && !string.Equals(text, expectedAssemblyName, StringComparison.Ordinal))
		{
			error = "loaded assembly name mismatch for '" + expectedAssemblyName + "'";
			return false;
		}
		string location = assembly.Location;
		string text2 = (string.IsNullOrWhiteSpace(authoritativePath) ? location : authoritativePath);
		if (string.IsNullOrWhiteSpace(text2) || !Path.IsPathFullyQualified(text2))
		{
			error = "loaded assembly path is unavailable for '" + text + "'";
			return false;
		}
		text2 = Path.GetFullPath(text2);
		if (!string.IsNullOrWhiteSpace(location) && !string.Equals(Path.GetFullPath(location), text2, StringComparison.OrdinalIgnoreCase))
		{
			error = "plugin registry path differs from the loaded assembly path for '" + text + "'";
			return false;
		}
		using FileStream fileStream = new FileStream(text2, FileMode.Open, FileAccess.Read, FileShare.Read, 65536, FileOptions.SequentialScan);
		if (fileStream.Length <= 0 || fileStream.Length > 134217728)
		{
			error = "loaded assembly byte length is invalid for '" + text + "'";
			return false;
		}
		using (PEReader pEReader = new PEReader(fileStream, PEStreamOptions.LeaveOpen))
		{
			...
			{
				error = "loaded assembly metadata is unavailable for '" + text + "'";
				return false;
			}
			MetadataReader metadataReader = pEReader.GetMetadataReader();
			string a = metadataReader.GetString(metadataReader.GetAssemblyDefinition().Name);
			Guid guid = metadataReader.GetGuid(metadataReader.GetModuleDefinition().Mvid);
			if (!string.Equals(a, text, StringComparison.Ordinal) || guid != assembly.ManifestModule.ModuleVersionId)
			{
				error = "loaded assembly path does not match its in-memory module for '" + text + "'";
				return false;
			}
		}
		fileStream.Position = 0L;
		using SHA256 sHA = SHA256.Create();
		sha256 = ToLowerHex(sHA.ComputeHash(fileStream));
	...
	{
		string text = ReadBoundedPvpString(reader, 64, field);
		if (!IsLowercasePvpSha256(text))
		{
	...
	{
		if (string.Equals(identity.CompanionPluginGuid, "none", StringComparison.Ordinal))
		{
			if (!string.Equals(identity.CompanionPluginVersion, "none", StringComparison.Ordinal) || !string.Equals(identity.CompanionSha256, "none", StringComparison.Ordinal) || !string.Equals(identity.CompanionReadyMarkerName, "none", StringComparison.Ordinal) || !string.Equals(identity.CompanionFailureMarkerName, "none", StringComparison.Ordinal))
			{
				throw new InvalidOperationException("companion none identity is internally inconsistent");
			}
		}
		else if (string.IsNullOrWhiteSpace(identity.CompanionPluginGuid) || string.IsNullOrWhiteSpace(identity.CompanionPluginVersion) || string.IsNullOrWhiteSpace(identity.CompanionReadyMarkerName) || string.IsNullOrWhiteSpace(identity.CompanionFailureMarkerName) || !string.Equals(identity.CompanionPluginGuid, identity.CompanionPluginGuid.Trim(), StringComparison.Ordinal) || !string.Equals(identity.CompanionPluginVersion, identity.CompanionPluginVersion.Trim(), StringComparison.Ordinal) || !string.Equals(identity.CompanionReadyMarkerName, identity.CompanionReadyMarkerName.Trim(), StringComparison.Ordinal) || !string.Equals(identity.CompanionFailureMarkerName, identity.CompanionFailureMarkerName.Trim(), StringComparison.Ordinal) || !IsLowercasePvpSha256(identity.CompanionSha256) || string.Equals(identity.CompanionReadyMarkerName, identity.CompanionFailureMarkerName, StringComparison.Ordinal))
		{
			throw new InvalidOperationException("companion SHA-256 or marker identity is invalid");
		}
	}

	private static bool PvpCompanionIdentityMatches(PvpPeerIdentity offer, RuntimeBinaryIdentity local)
	{
		if (string.Equals(offer.CompanionPluginGuid, local.CompanionPluginGuid, StringComparison.Ordinal) && string.Equals(offer.CompanionPluginVersion, local.CompanionPluginVersion, StringComparison.Ordinal) && string.Equals(offer.CompanionSha256, local.CompanionSha256, StringComparison.Ordinal) && string.Equals(offer.CompanionReadyMarkerName, local.CompanionReadyMarkerName, StringComparison.Ordinal))
		{
			return string.Equals(offer.CompanionFailureMarkerName, local.CompanionFailureMarkerName, StringComparison.Ordinal);
		}
		return false;
	}
	...
	{
		if (reader == null || reader.Remaining < 3 || reader.Remaining > 8192)
		{
			throw new InvalidOperationException("PVP agreement envelope length is invalid");
		}
	}
	...
	private static void RequirePvpEnvelopeConsumed(NetworkReader reader)
	{
		...
		{
			throw new InvalidOperationException("PVP agreement envelope has trailing data");
		}
	}

	private static string ReadBoundedPvpString(NetworkReader reader, int maximumLength, string field, bool allowEmpty = false)
	{
		string text = NetworkReaderExtensions.ReadString(reader) ?? string.Empty;
	...
	{
		return Stopwatch.GetTimestamp() + (long)(seconds * (double)Stopwatch.Frequency);
	}
